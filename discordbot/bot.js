...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("RPFrogs Server Information", "https://i.kym-cdn.com/entries/icons/original/000/000/142/feelsgoodman.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ RPFrogs' : '⚫ RPFrogs')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHow do I join the server?','Direct Connect: F8 to access FiveM Console -> and type connect 62.171.159.104         Browser: Click [here](https://cfx.re/join/5b74lz) to be automatically connected via your browser.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('Roofstad', 'https://www.twitch.tv/xqcow');
    bot.user.setActivity('RPFrogs Server',{'url':'https://www.twitch.tv/xqcow','type':'WATCHING'});
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('The bug report has been forwarded to the staff team.')
          .setTimestamp(new Date());
