...
RegisterNetEvent('drp-base:PolyZoneUpdate')
AddEventHandler('drp-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "drp-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('drp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('drp-polyzone:GaragePolyEnter')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('drp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('drp-polyzone:GaragePolyLeft')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('drp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('drp-polyzone:Open')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('drp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('drp-polyzone:stop')
            end
        end
