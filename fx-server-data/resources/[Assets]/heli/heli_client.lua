...
local toggle_helicam = 101	 -- control id of the button by which to toggle the helicam mode. Default: INPUT_CONTEXT (H)
...
function rappelJumpSound()
    TriggerEvent('InteractSound_CL:PlayOnOne', 'jumpland', 1.0)
end

function rappelSound()
	if rappelSoundPlayed > 340 then
        TriggerEvent('InteractSound_CL:PlayOnOne', 'rapell', 1.0)
		rappelSoundPlayed = 0
	end
end
            ...
            while helicam and not IsEntityDead(lPed) and
                ...
                if vehicle == nil then
                    ...
                    if useMilesPerHour then
                        ...
                        vehspd = string.format(
                                     "%." .. (numDecimalPlaces or 0) .. "f",
                                     GetEntitySpeed(vehicle) * 3.6) .. " mph"
                    end
...
function IsPlayerInBCSO()
	local lPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, bcso_hash)
end
