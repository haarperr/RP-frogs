...
local function handleConnectionEvent(pChannel)
  ...
  if newChannel < 1.0 then
    ...
    exports['drp-voice']:addPlayerToRadio(newChannel)
  end
end
...
function hasRadio()
    if exports["drp-inventory"]:hasEnoughOfItem("radio",1,false) or exports["drp-inventory"]:hasEnoughOfItem("civradio",1,false) or exports["drp-inventory"]:hasEnoughOfItem("emsradio",1,false) then
      return true
    else
      return false
    end
...
RegisterNUICallback('volumeUp', function(data, cb)
  ...
  exports['drp-voice']:setRadioVolume(radioVolume)
  exports['drp-voice']:getRadioVolume()
...
RegisterNetEvent('drp-radio:ResetUI')
AddEventHandler('drp-radio:ResetUI', function()
    GuiOpened = false
    SetNuiFocus(false,false)
    SendNUIMessage({open = false})
    TriggerEvent("animation:radio",GuiOpened)
...
Citizen.CreateThread(function()
  exports["drp-binds"]:registerKeyMapping("", "Radio", "Radio Volume Up", "+RadioVolumeUp", "-RadioVolumeDown", "PAGEUP")

  exports["drp-binds"]:registerKeyMapping("", "Radio", "Radio Volume Down", "+RadioVolumeDown", "-RadiopVolumeDown", "PAGEDOWN")
end)
...
RegisterNetEvent('drp-radio:set_chan_1')
AddEventHandler('drp-radio:set_chan_1', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    pRadioChannel = true
    SendNUIMessage({set = true, setChannel = chan})
...
RegisterNetEvent('drp-radio:set_chan_2')
AddEventHandler('drp-radio:set_chan_2', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    pRadioChannel = true
    SendNUIMessage({set = true, setChannel = chan})
...
RegisterNetEvent('drp-radio:set_chan_3')
AddEventHandler('drp-radio:set_chan_3', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
...
RegisterNetEvent('drp-radio:set_chan_4')
AddEventHandler('drp-radio:set_chan_4', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    exports['drp-voice']:addPlayerToRadio(4)
    exports["drp-voice"]:setVoiceProperty("radioEnabled", true)
    PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
RegisterNetEvent('drp-radio:set_chan_5')
AddEventHandler('drp-radio:set_chan_5', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    exports['drp-voice']:addPlayerToRadio(5)
    exports["drp-voice"]:setVoiceProperty("radioEnabled", true)
    PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
