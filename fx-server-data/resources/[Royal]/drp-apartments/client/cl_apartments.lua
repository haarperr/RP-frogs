...
RegisterNetEvent('drp-binds:keyEvent')
AddEventHandler('drp-binds:keyEvent', function(name,onDown)
...
function Apart.locksMotel()
    ...
    TriggerEvent('InteractSound_CL:PlayOnOne', 'keydoors', 1.0)
...
function Apart.enterMotel(roomNumber,roomType,isSpawn)
    ...
    TriggerServerEvent('drp-apartments:SetNewRouting', roomNumber)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn)
    DoScreenFadeOut(1)
    TriggerServerEvent('drp-apartments:GetRouting')
    TriggerEvent("inhotel",true)
    local name = ""
    name = Apart.FindCurrentRoom(roomType)

    local isBuiltCoords = exports["drp-build"]:getModule("func").buildRoom(name,numMultiplier,false)
    ...
    if isBuiltCoords then
        ...
        if isSpawn then
            Apart.WakeFromBed()
            TriggerEvent("drp-spawn:characterSpawned")
        else
...
function Apart.leaveApartment()
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["drp-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["drp-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["drp-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
    ...
    TriggerServerEvent('drp-apartments:SetOldRouting')
    Citizen.Wait(100)
    TriggerServerEvent('drp-apartments:GetRouting')
end
...
--     exports["drp-build"]:getModule("func").CleanUpArea()
--     local cid = exports["isPed"]:isPed("cid")
--     TriggerServerEvent("drp-jobmanager:onCharSwap", cid)
--     Citizen.Wait(1000)   
...
function garageToWorld()
    ...
    exports["drp-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
function Apart.WakeFromBed()
    local bedOffset = exports["drp-build"]:getModule("func").getCurrentBed()
...
AddEventHandler('apartment:removeFromBuilding', function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["drp-build"]:getModule("func").isInBuilding() then 
...
RegisterUICallback("drp-apartments:handler", function(data, cb)
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        -- exports["drp-ui"]:showContextMenu(MenuData["apartment_check"])
    end
end)
