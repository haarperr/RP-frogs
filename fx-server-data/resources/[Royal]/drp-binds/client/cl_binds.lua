-- Key binds:
-- Usage:
-- exports["drp-binds"]:registerKeyMapping(category, description, onKeyDownCommand, onKeyUpCommand, defaultKey)
...
AddEventHandler("drp-binds:should-execute", function(shouldExecute)
...
exports('registerKeyMapping', function(name, category, description, onKeyDownCommand, onKeyUpCommand, default, event, type)
    ...
    RegisterCommand(cmdStringDown, function()
      ...
      if event then TriggerEvent("drp-binds:keyEvent", name, true) end
      ExecuteCommand(onKeyDownCommand)
    end, false)
    RegisterCommand(cmdStringUp, function()
      if not shouldExecuteBind then return end
      if event then TriggerEvent("drp-binds:keyEvent", name, false) end
