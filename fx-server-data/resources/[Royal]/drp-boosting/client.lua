...
AddEventHandler('trackerGoBr', function()
  if IsPedInVehicle(PlayerPedId(),spawnedVeh, false) then
    TriggerEvent('drp-dispatch:boostTrackerPing', spawnedVeh)
  end
end)
...
RegisterNetEvent('drp-boosting:removeTracker')
AddEventHandler('drp-boosting:removeTracker', function()
  local curVeh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  while true do
    Citizen.Wait(5)
      if IsPedInVehicle(PlayerPedId(), spawnedVeh, false) and CallingCops == true and StartedBoost == true then
        ...
        TriggerEvent('drp-dispatch:boostTrackerPing',true,spawnedVeh)
      if DispatchDelayTimer <= 0 then
        TriggerEvent('drp-dispatch:boostTrackerPing',false,spawnedVeh)
        return end
    else
      Wait(500)
    end
...
RegisterNetEvent('drp-boosting:delayTracker')
AddEventHandler('drp-boosting:delayTracker', function()
  if IsPedInVehicle(PlayerPedId(), spawnedVeh, false) and StartedBoost then
  local trackerGame = exports['drp-minigame']:Open()
  if trackerGame then 
