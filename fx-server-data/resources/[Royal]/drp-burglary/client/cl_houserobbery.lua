...
AddEventHandler('houseRobberies:attempt', function()
 ...
 if exports['drp-inventory']:hasEnoughOfItem('advlockpick', 1, false) and not isRobbing then
  local playerCoords = GetEntityCoords(PlayerPedId(), true)
  ...
  TriggerEvent("drp-hud:updateStress",true,50)
		FreezeEntityPosition(PlayerPedId(),true)
    if exports["drp-lockpicking"]:lockpick(100,5,2,10) == 100 then
     isLockpicking = false
     pedSpawned = false
 ...
 elseif exports['drp-inventory']:hasEnoughOfItem('advlockpick', 0, false) and not isRobbing then
...
AddEventHandler('houseRobberies:createHouse', function(id)
 ...
 while isRobbing do
  ...
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
    exports['drp-textui']:showInteraction("[E] to Exit House")
  ...
  local myJob = exports["isPed"]:isPed("myJob")
    ...
    for id,v in pairs(robbableHouses) do
      if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
        ...
        local finished = exports["drp-taskbar"]:taskBar(5000,"Breaching The House",false,false,false)
        if (finished == 100) then
          TriggerEvent('houseRobberies:createHouse', id)
        end
      end
