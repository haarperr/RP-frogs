			...
			for i = 1, #vehicleWashStation do
				...
				if #(GetEntityCoords(PlayerPedId()) - vector3(garageCoords2[1], garageCoords2[2], garageCoords2[3])) < 5 then
					if isCleaning == false then
						exports['drp-textui']:showInteraction("Press [ENTER] to clean your vehicle!")
					else
						exports['drp-textui']:hideInteraction()
					end
					if(IsControlJustPressed(1, Key)) and isCleaning == false then
						TriggerServerEvent('drp-carwash:checkmoney')
					end
				end
			end
...
RegisterNetEvent('drp-carwash:success')
AddEventHandler('drp-carwash:success', function(price)
	TriggerEvent('DoLongHudText', 'Vehicle Clean $' .. price ..'!')
	FreezeEntityPosition(GetVehiclePedIsUsing(PlayerPedId()), true)
	isCleaning = true
	local finished = exports["drp-taskbar"]:taskBar(10000,"Cleaning Vehicle",false,false,playerVeh)
...
RegisterNetEvent('drp-carwash:notenoughmoney')
AddEventHandler('drp-carwash:notenoughmoney', function(moneyleft)
