...
Citizen.CreateThread(function()
  for _, v in pairs(LAUNDRY_COORDS) do
    exports["drp-polyzone"]:AddBoxZone("casino_hotel_laundry_machine", v[1], v[2], v[3], v[4])
  end
  exports["drp-interact"]:AddPeekEntryByPolyTarget("casino_hotel_laundry_machine", {{
    id = "casino_laundry_interact",
    label = "Do Laundry",
    icon = "money-bill-alt",
    event = "drp-casino:hotel:doLaundry",
  }}, { distance = { radius = 1.5 } })
end)

AddEventHandler("drp-casino:hotel:doLaundry", function(p1, p2, pContext)
  local laundryId = pContext.zones["casino_hotel_laundry_machine"].id
  local coins = exports["drp-inventory"]:getItemsOfType("casinogoldcoin", 1, true)
  local coin = coins[1]
  if not coin then
  ...
  local canLaunder = RPC.execute("drp-casino:hotel:canUseLaundry", laundryId, roomId, true)
  ...
  local finished = exports["drp-taskbar"]:taskBar(15000, [[Adding "Laundry"...]])
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end
  RPC.execute("drp-casino:hotel:canUseLaundry", laundryId, roomId)
end)
