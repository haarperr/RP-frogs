...
Citizen.CreateThread(function()
    exports["drp-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=true,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("drp-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == RoyalChopCar then
        ...
        IsInChopZone = true
        RoyalStartChopping()
        exports['drp-textui']:showInteraction('[E] Start Chopping')
    end
end)

function RoyalStartChopping()
	Citizen.CreateThread(function()
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("drp-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('drp-chopshop:clock-in')
AddEventHandler('drp-chopshop:clock-in', function()
    if CoolDownActive then
...
RegisterNetEvent('drp-chopshop:clock-out')
AddEventHandler('drp-chopshop:clock-out', function()
    if ClockedIntoChop then
...
function RoyalCreateChopCar()
	if DoesEntityExist(RoyalChopCar) then
	    SetVehicleHasBeenOwnedByPlayer(RoyalChopCar,false)
		SetEntityAsNoLongerNeeded(RoyalChopCar)
		DeleteEntity(RoyalChopCar)
	end

    local RoyalChopVehicle = GetHashKey(ChopCars[math.random(#ChopCars)])
    RequestModel(RoyalChopVehicle)
    while not HasModelLoaded(RoyalChopVehicle) do
        Citizen.Wait(0)
    end
