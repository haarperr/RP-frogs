...
RegisterNetEvent('drp-doors:initial-lock-state')
AddEventHandler('drp-doors:initial-lock-state', function(pDoors)
    doors = pDoors
...
AddEventHandler('drp-doors:change-lock-state', function(pDoorId, pDoorLockState, pDoorForceUnlock)
...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState then
            exports["drp-textui"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.5 then
                    ...
                    if hasAccess and not isHidden then
                        exports["drp-textui"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    else
                    end
                else
                    idle = 100
            ...
            if currentDoorId ~= nil and hasAccess and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                TriggerServerEvent("drp-doors:change-lock-state", currentDoorId, not currentDoorLockState)
            end

            Wait(idle)
        end

        exports["drp-textui"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
Citizen.CreateThread(function()
    while true do
        ...
        local entity, entityType, entityCoords = exports['drp-targetinteract']:GetEntityPlayerIsLookingAt(3.0, 0.2, 286, PlayerPed)
        ...
        if entity and entityType ~= 0 then
            if entity ~= CurrentTarget then
                ...
                TriggerEvent('drp-target:inFront', CurrentTarget, entityType, entityCoords)
            end
        elseif CurrentTarget then
            CurrentTarget = nil
            TriggerEvent('drp-target:inFront', CurrentTarget)
        end
...
AddEventHandler("drp-doors:doorKeyFob", function()
    ...
    TriggerServerEvent("drp-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)

Citizen.CreateThread(function()
    exports["drp-polyzone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["drp-polyzone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    TriggerServerEvent("drp-doors:request-lock-state")
end)

RegisterNetEvent("drp-doors:add")
AddEventHandler("drp-doors:add", function(pArgs, pEntity, pContext)
  if GetConvar("sv_environment", "prod") == "debug" then
    TriggerServerEvent("drp-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
  end
end)
