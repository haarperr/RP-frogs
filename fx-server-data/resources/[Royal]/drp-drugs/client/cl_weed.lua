...
RegisterNetEvent('drp-weed:knownCrops')
AddEventHandler('drp-weed:knownCrops', function(sentInfo, currentTime)
...
RegisterNetEvent('drp-weed:plantSeed')
AddEventHandler('drp-weed:plantSeed', function(sentType)
    if not isBusy then
        if exports["drp-inventory"]:hasEnoughOfItem("farmhoe",1,false) then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if shouldBreak == true then
                ...
                DeleteEntity(CreatedObject)
                local finished = exports["drp-taskbar"]:taskBar(math.random(8000, 12000),"Planting",false,false)
...
RegisterNetEvent('drp-weed:destroyPlant')
AddEventHandler('drp-weed:destroyPlant', function(coords)
    if not isBusy then
        isBusy = true
                ...
                CreateThread(function()
                    ...
                    while true do
                        ...
                        if GetGameTimer() >= endTime then
                            TriggerServerEvent("drp-weed:killplant", knownCrops[knownInfo['id']]['knownId'])
                            break
                        end
                    end
                end)
...
function NewPlant(sentType, coordsX, coordsY, coordsZ)
	TriggerServerEvent("drp-weed:newplant", coordsX, coordsY, coordsZ, sentType)
end 

RegisterNetEvent('drp-weed:updatePlant')
AddEventHandler('drp-weed:updatePlant', function(updateType, sentId, sentGrowth, sentTime, sentStatus, sentNewInfo, currentTime)
...
local TyPes = {
    ['maleseed'] = 'Sex Grenade',
    ['femaleseed'] = 'Cherry Pie',
}
            ...
            if cropDist < 1.8 then
                ...
                if cropDist < 2 then
                    if IsControlJustReleased(0, 304) then
                        if knownCrops[closest]["growth"] >= 100 then
                            local finished = exports["drp-taskbar"]:taskBar(1000, "Picking Plant", false, false)
                            if (finished == 100) then
                                TriggerServerEvent('drp-weed:killplant', knownCrops[closest]["knownId"])
                        ...
                        elseif num == 2 then
                            if exports["drp-inventory"]:hasEnoughOfItem("wateringcan",1,false) then
                                if math.random(1,5) == 5 then
                                TriggerEvent("inventory:removeItem", "wateringcan", 1)
                                TriggerEvent("DoLongHudText", "Your watering can ran out of water", 2)
                                end
                                local new = math.ceil(knownCrops[closest]["growth"] + math.random(10,15))
                                TriggerServerEvent("drp-weed:updateStatus", 'update', knownCrops[closest]["knownId"], new, 'water')
                            else
                                TriggerEvent("DoLongHudText", "You need a watering can.", 2)
                            end
               
                        elseif num == 3 then
                            if exports["drp-inventory"]:hasEnoughOfItem("fertilizer",1,false) then
                                TriggerEvent("inventory:removeItem", "fertilizer", 1)
                                local new = math.ceil(knownCrops[closest]["growth"] + math.random(10,15))
                                TriggerServerEvent("drp-weed:updateStatus", 'update', knownCrops[closest]["knownId"], new, 'fertilizer')
                            else
