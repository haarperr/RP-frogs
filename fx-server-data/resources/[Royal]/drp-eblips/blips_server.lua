...
RegisterNetEvent('drp-eblips:server:registerSourceName')
AddEventHandler('drp-eblips:server:registerSourceName', function(source, name)
    if not source_cache[source] then source_cache[source] = { } end
    source_cache[source].name = name
end)

RegisterNetEvent('drp-eblips:server:registerPlayerBlipGroup')
AddEventHandler('drp-eblips:server:registerPlayerBlipGroup', function(source, group)
...
RegisterNetEvent('drp-eblips:server:removePlayerBlipGroup')
AddEventHandler('drp-eblips:server:removePlayerBlipGroup', function(source, group)
...
RegisterNetEvent('drp-eblips:server:registerPlayerTempBlipGroup')
AddEventHandler('drp-eblips:server:registerPlayerTempBlipGroup', function(source, group, time)
    TriggerEvent('drp-eblips:server:registerPlayerBlipGroup', source, 'police')
    SetTimeout(time, function()
        TriggerEvent('drp-eblips:server:removePlayerBlipGroup', source, 'police')
    end)
end)
            ...
            Citizen.CreateThread(function()
                for source, is_valid_member in pairs(blip.members) do
                    if is_valid_member then -- more sanity checking
                        TriggerClientEvent('drp-eblips:client:syncMyBlips', source, blips_data)
                        Citizen.Wait(100)
                    end
                end
            end)
