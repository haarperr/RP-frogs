...
RegisterNetEvent("drp-garages:attempt:spawn", function(data, pRealSpawn)
    if not pRealSpawn then
      ...
      local vehicle = SpawnVehicle(data.model, exports['drp-menu']:currentGarage(), data.fuel, data.customized, data.plate, true, data.engine_damage, data.body_damage)
     ...
     local vehicle = SpawnVehicle(data.model, exports['drp-menu']:currentGarage(), data.fuel, data.customized, data.plate, false, data.engine_damage, data.body_damage)
     SetVehicleNeedsToBeHotwired(vehicle, false)
     SetVehicleHasBeenOwnedByPlayer(vehicle, true)
     SetEntityAsMissionEntity(vehicle, true, true)
     SetVehicleIsStolen(vehicle, false)
