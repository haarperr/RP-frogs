...
RegisterNetEvent('drp-heists:start_loop')
AddEventHandler('drp-heists:start_loop', function()
    pickup = true
    TriggerEvent("drp-heists:pick_cash")
    Wait(180000)
    Wait(additionalWait)
    pickup = false
end)

RegisterNetEvent('drp-heists:pick_cash')
AddEventHandler('drp-heists:pick_cash', function()
...
RegisterNetEvent('drp-heists:spawn_peds')
AddEventHandler('drp-heists:spawn_peds', function(veh)
...
RegisterNetEvent('drp-heists:start_hitting_truck')
AddEventHandler('drp-heists:start_hitting_truck', function(veh)
    TriggerEvent('drp-dispatch:bank_truck_robbery')
    attempted = veh
    SetEntityAsMissionEntity(attempted,true,true)
    local plate = GetVehicleNumberPlateText(veh)
    TriggerServerEvent("drp-heists:check_if_robbed",plate)
end)

RegisterNetEvent('sec:AllowHeist')
AddEventHandler('sec:AllowHeist', function()
    TriggerServerEvent('banktruckrobbery:log')
    TriggerEvent("drp-heists:spawn_peds",attempted)
    SetVehicleDoorOpen(attempted, 2, 0, 0)
    SetVehicleDoorOpen(attempted, 3, 0, 0)
    TriggerEvent("drp-heists:start_loop")
