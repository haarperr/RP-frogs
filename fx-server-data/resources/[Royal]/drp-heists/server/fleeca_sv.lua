...
AddEventHandler("drp-fleeca:rewardCash", function()
    local src = source
    local user = exports["drp-base"]:getModule("Player"):GetUser(src)
    local cash = math.random(3333, 8888) -- 3333 - 8888
    TriggerClientEvent('player:receiveItem',src, "markedbills", math.random(20,40))
    user:addMoney(cash)
    if math.random(100) < 33 then -- Blue Dongle Chance 33%
        TriggerClientEvent('player:receiveItem',src, "heistusb1", 1)
        end
    if math.random(100) < 15 then -- Big Bag Chance 15%
