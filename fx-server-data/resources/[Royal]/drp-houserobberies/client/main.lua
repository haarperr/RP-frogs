...
Citizen.CreateThread(function()
	local result = RPC.execute('drp-houserobbery:enter_property')
	coord = true
	print(result)
end)
					...
					if #(GetEntityCoords(PlayerPedId()) - objCoords) <= items[i].dist and not items[i].robado then
						...
						if IsControlJustPressed(0,38) and not charging then
							...
							if items[i].anim == 'cajafuerte' then
								...
								TriggerEvent('drp-houserobber:animations',items[i].anim,items[i].delete,i)
							end
						end
					end
				end
	...
	if not working then
		if exports['drp-inventory']:hasEnoughOfItem('vpnxj', 1) then
			if exports['drp-inventory']:hasEnoughOfItem('advlockpick', 1) then
				...
				working = true
				enterCoords = Houses()
				TriggerEvent('phone:addnotification', 'Boss', "Ill have a job for you shortly.")
				...
				TriggerEvent('DoLongHudText', 'This lock seems pretty advanced', 2)
			end
		else
...
RegisterNetEvent('drp-houserobbery:make_noise')
AddEventHandler('drp-houserobbery:make_noise', function()
	...
	while stealing do		
		if within then	
			...
			if noise > 100 then
				-- dispatch for pd
				TriggerEvent('drp-dispatch:houserobbery')
				Citizen.Wait(300000)
			end
		end
...
RegisterCommand('raid', function()
	if PlayerJob == "police" or "state" or "sheriff" then
		TriggerEvent('drp-houserobbery:police_enter')	
	end
end)

RegisterNetEvent('drp-houserobbery:police_enter')
AddEventHandler('drp-houserobbery:police_enter', function()
	if PlayerJob == "police" or "state" or "sheriff" then
	...
	while working do
		local espera = 1500
			...
			if distancia < 3 and isnight() then
				espera = 5
				exports['drp-textui']:showInteraction('[H] To Enter House')
					if IsControlJustReleased(0,74) and distancia < 3 and firstentry then
						RemoveBlip(HouseRobberyLocation)
						if exports['drp-inventory']:hasEnoughOfItem('advlockpick', 1) then
							local finished = exports["drp-ui"]:taskBarSkill(5000,math.random(100, 200))
							if (finished == 100) then
								local finished2 = exports["drp-ui"]:taskBarSkill(5000,math.random(200,400))
								if (finished2 == 100) then
									local finished3 = exports["drp-ui"]:taskBarSkill(5000,math.random(50,100))
									if (finished3 == 100) then
										local finished4 = exports["drp-ui"]:taskBarSkill(5000,math.random(200,400))
										if (finished4 == 100) then
											local finished5 = exports["drp-ui"]:taskBarSkill(5000,math.random(200,400))
...
function EnterHouse(police)
	if not stealing then
		...
		stealing = true
		if not police then
			TriggerEvent('drp-houserobbery:make_noise')
		end
	else
...
--RegisterNetEvent('client:enterhouse')
--AddEventHandler('client:enterhouse', function()
	--entrance()
	--en

RegisterNetEvent('drp-houserobber:animations')
AddEventHandler('drp-houserobber:animations', function(anim, delete, i)
	local ped = PlayerPedId()
	...
	elseif anim == 'normal' then
		FreezeEntityPosition(ped,true)	
		AnimBolso()
		TriggerEvent('drp-houserobbery:common_items')
		if delete then
			DeleteEntity(items[i].objeto)
		end
...
function idklol()
	...
	if vehicle then
		...
		exports['drp-textui']:showInteraction('Cannot Place Inside Of Trunk', 'error')
		Citizen.Wait(2500)
		exports['drp-textui']:hideInteraction()
	end
	return false
