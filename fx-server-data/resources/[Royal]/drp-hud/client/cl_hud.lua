...
RegisterNetEvent("drp-hud:EnableHud")
AddEventHandler("drp-hud:EnableHud", function(status)
    isLoggedIn = status
end)
...
AddEventHandler('drp-admin:currentDevmode', function(test)
    developer = test
end)

RegisterNetEvent('drp-admin:currentDebug')
AddEventHandler('drp-admin:currentDebug', function(status)
    debug = status
end)
...
RegisterNetEvent("drp-hud:changeRange")
AddEventHandler("drp-hud:changeRange", function(pRange)
    voice = pRange or 2
end)
...
Citizen.CreateThread(function ()
	while true do
        ...
        local pRadioActive = exports['drp-voice']:pRadioActive()
        SendNUIMessage({
            action = "voiceupdate", 
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            SendNUIMessage({
                ...
                dev = developer,
                dev2 = debug,
                nitro = exports["drp-nitro"]:HowmuchNitro(),                
                casino = tiempo,
                inVehicle = inveh,
                cinturon = harnessDur,
...
RegisterNetEvent('drp-hud:ChangeThirst')
AddEventHandler('drp-hud:ChangeThirst', function(amount)
...
RegisterNetEvent('drp-hud:ChangeHunger')
AddEventHandler('drp-hud:ChangeHunger', function(amount)
	if isLoggedIn == true then
...
RegisterCommand('hud', function()
    TriggerEvent('drp-hud:open-hud')
end)

RegisterNetEvent('drp-hud:open-hud')
AddEventHandler('drp-hud:open-hud', function()
...
RegisterNetEvent('drp-gas:checkpump')
AddEventHandler('drp-gas:checkpump', function()
...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    local finished = exports["drp-taskbar"]:taskBar(timer,"Refueling")
    local veh = getVehicleClosestToMe()
...
RegisterNetEvent('drp-admin:maxstats')
AddEventHandler('drp-admin:maxstats', function()
    currentValues["thirst"] = 100
	currentValues["hunger"] = 100 
    TriggerEvent("heal", PlayerPedId())
    TriggerEvent("Hospital:HealInjuries", PlayerPedId(),true) 
    TriggerServerEvent("drp-death:reviveSV", source)
...
RegisterNetEvent("drp-hud:updateStress")
AddEventHandler("drp-hud:updateStress", function(status, amount)
    if status == true then
        pStress = pStress + amount
        TriggerEvent("DoShortHudText",'Stress Gained',6)
        Wait(1000)
        TriggerServerEvent('drp-hud:UpdateStress_SV', pStress)
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('drp-hud:EnableHud', false)
    TriggerEvent('closeInventoryGui')
    TriggerEvent('drp-banking:ResetUI')
    TriggerEvent('drp-radio:ResetUI')
    TriggerEvent('drp-menu:ResetUI')
    TriggerEvent('drp-interact:ResetUI')
    TriggerEvent('drp-mdt:ResetUI')
    TriggerEvent('phone:reset')
    TriggerEvent('clothing:close')
    exports['drp-textui']:showInteraction("Restarting UI .")
    Wait(1000)
    exports['drp-textui']:showInteraction("Restarting UI ..")
    Wait(1000)
    exports['drp-textui']:showInteraction("Restarting UI ...")
    Wait(1000)
    exports['drp-textui']:showInteraction("UI Successfully Restarted")
    Wait(1000)
...
function removeAttachedProp2()
  ...
  RegisterNetEvent("drp-hud:jerry_can:refuel")
  AddEventHandler("drp-hud:jerry_can:refuel",function()
      vehicleCurrentFuel = DecorGetInt(veh, "GetVehicleCurrentFuel")
      if exports['drp-inventory']:hasEnoughOfItem('883325847', 1) then
        local timer = (100 - vehicleCurrentFuel) * 400
        refillVehicle()
        local finished = exports["drp-taskbar"]:taskBar(timer,"Refueling")
        local veh = getVehicleClosestToMe()
