...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags['isJobEmployer'] then
        pContext.job = exports['drp-jobs']:GetNPCJobData(npcId)
    end
end
...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports['drp-flags']:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        --context.meta = exports['drp-vehicles']:GetVehicleMetadata(pEntity)
        context.flags = exports['drp-flags']:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports['drp-flags']:GetObjectFlags(pEntity)
    end
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports['drp-vehicles']:HasVehicleKey(pEntity)
...
RegisterNetEvent("drp-interact:bustVehicleOwnerShipCache", function(pEntity)
...
RegisterNetEvent('drp-jobs:jobChanged')
AddEventHandler('drp-jobs:jobChanged', function(pJobId)
    CurrentJob = pJobId
...
AddEventHandler("drp-polyzone:enter", function(zone, data)
    if zone == "drp-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
AddEventHandler("drp-interact:openFridge", function()
  TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("drp-interact:grabDrink", function(args, entity, context)
