...
local randomDrivingActions = {7, 8, 10, 11, 32}

local randomEffects = {"lsd", "weed", "cocaine", "crack", "metamorphine"}

local WillOD = 0

local parachuteConfig = {
    ["-72657034"] = {replace = p_parachute1_mp_s, tint = 6},
    ["airxchute"] = {replace = p_parachute1_mp_dec, tint = 1},
    ["airbornechute"] = {replace = p_parachute2_mp_dec, tint = 1}
}
...
AddEventHandler("drp-jobmanager:playerBecameJob", function(job) myJob = job end)

AddEventHandler("drp-katana:removeEquippedKatana",
                function() isKatanaEquipped = false end)

local fruits = {
    "apple", "banana", "cherry", "coconut", "grapes", "kiwi", "lemon", "peach",
...
local validDogModels = {[a_c_chop] = true, [a_c_husky] = true}
...
function GetRandomString(lenght)
    ...
    for char in chars:gmatch "." do table.insert(charTable, char) end
...
function RegisterItemCallback(itemName, cb) ItemCallbacks[itemName] = cb end
...
Citizen.CreateThread(function()
    exports["drp-binds"]:registerKeyMapping("", "Vehicle", "Door Keyfob",
                                            "+useKeyFob", "-useKeyFob", "Y")
...
AddEventHandler('inventory-jail', function(startPosition, cid, name)
    if (hasEnoughOfItem("okaylockpick", 1, false)) then
        ...
        if inPoly then
            TriggerServerEvent("server-inventory-open", startPosition, cid, "1",
                               name);
        end
    end
end)

local stolenItems = {["stolentv"] = true}
...
AddEventHandler('inventory:DegenLastUsedItem', function(percent)
    ...
    print("Degen applied to " .. LastUsedItemId .. " ID: " .. LastUsedItem ..
              " at %" .. percent)
    TriggerServerEvent("inventory:degItem", LastUsedItem, percent,
                       LastUsedItemId, cid)
end)

RegisterNetEvent('RunUseItem')
AddEventHandler('RunUseItem',
                function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if justUsed then
        ...
        if retardCounter > 10 and retardCounter > lastCounter + 5 then
            ...
            TriggerServerEvent("exploiter", "Tried using " .. retardCounter ..
                                   " items in < 500ms ")
        end
        return
    end
    ...
    if (not hasEnoughOfItem(itemid, 1, false)) then
        TriggerEvent("DoLongHudText",
                     "You dont appear to have this item on you?", 2)
        justUsed = false
        retardCounter = 0
        lastCounter = 0
        return
    ...
    if parachuteConfig[itemid] and tonumber(ItemInfo.quality) > 0 and
        not IsPedInParachuteFreeFall(player) and not IsPedFalling(player) and
        (GetPedParachuteState(player) == -1 or GetPedParachuteState(player) == 0) then
        ...
        TriggerEvent("equipWeaponID", "-72657034", ItemInfo.information,
                     ItemInfo.id)
    ...
    if itemid == "-1518444656" then
        if tonumber(ItemInfo.quality) > 0 then
            ...
            TriggerEvent("equipWeaponID", "1649403952", json.encode(katanaInfo),
                         ItemInfo.id)
            Citizen.CreateThread(function()
    ...
    if (isWeapon) then
        if tonumber(ItemInfo.quality) > 0 then
            TriggerEvent("equipWeaponID", itemid, ItemInfo.information,
                         ItemInfo.id)
        end
        justUsed = false
        retardCounter = 0
    ...
    if itemid == "smokegrenadeswat" or itemid == "smokegrenadenpa" then
        if tonumber(ItemInfo.quality) > 0 then
            TriggerEvent("equipWeaponID", -37975472, ItemInfo.information,
                         ItemInfo.id, itemid)
        end
        justUsed = false
        retardCounter = 0
    ...
    if itemid == "cursedkatanaweapon" then
        if tonumber(ItemInfo.quality) > 0 then
            ...
            TriggerEvent("equipWeaponID", "1692590063", json.encode(katanaInfo),
                         ItemInfo.id)
        end
        justUsed = false
        retardCounter = 0
    ...
    if itemid == "knuckle_chain" then
        if tonumber(ItemInfo.quality) > 0 then
            ...
            hiddenKeys[#hiddenKeys + 1] = "componentVariant"
            katanaInfo._hideKeys = hiddenKeys
            TriggerEvent("equipWeaponID", "3638508604", json.encode(katanaInfo),
                         ItemInfo.id)
        end
        justUsed = false
        retardCounter = 0
    ...
    if itemid == "gavel" then
        if tonumber(ItemInfo.quality) > 0 then
            ...
            hiddenKeys[#hiddenKeys + 1] = "componentVariant"
            katanaInfo._hideKeys = hiddenKeys
            TriggerEvent("equipWeaponID", "1317494643", json.encode(katanaInfo),
                         ItemInfo.id)
        end
        justUsed = false
        retardCounter = 0
    ...
    if itemid == "buddhamedalion" then buddhaMedalion() end
    ...
    if (not IsPedInAnyVehicle(player)) then
        if (itemid == "Suitcase") then TriggerEvent('attach:suitcase') end
        ...
        if (itemid == "Toolbox") then TriggerEvent('attach:toolbox') end
        if itemid == "wheelchair" then
            if not DoesEntityExist(wheelChair) then
                ...
                wheelChair = CreateVehicle(wheelChairModel,
                                           GetEntityCoords(PlayerPedId()),
                                           GetEntityHeading(PlayerPedId()),
                                           true, false)
                SetVehicleOnGroundProperly(wheelChair)
                SetVehicleNumberPlateText(wheelChair,
                                          "PILLBOX" .. math.random(9))
            ...
            elseif DoesEntityExist(wheelChair) and
                #(GetEntityCoords(wheelChair) - GetEntityCoords(PlayerPedId())) <
                3.0 and GetPedInVehicleSeat(wheelChair, -1) == 0 then
                Sync.DeleteVehicle(wheelChair)
                wheelChair = nil
            else
                TriggerEvent("DoLongHudText",
                             "Too far away from the wheelchair or someon is sitting in it !",
                             1)
            end
        end
    end
    ...
    if ItemCallbacks[itemid] and type(ItemCallbacks[itemid]) == 'function' then
        ...
        if options.remove then remove = true end
    end
    ...
    if (itemid == "nitrous") then TriggerEvent("carmod:nos") end
        ...
        if itemInfo == nil then
            ...
            if itemInfo ~= nil then
                if itemInfo.id ~= nil then
                    TriggerEvent("server-inventory-open", "1",
                                 "duffelbag" .. ItemInfo.id)
                end
            end
        end
    end

    if (itemid == "dojochain") then TriggerEvent('dojo:togglechain') end

    if (itemid == "jessemask") then TriggerEvent('jesse:togglemask') end

    if (itemid == "radio") then TriggerEvent('radioGui') end

    if (itemid == "mobilephone" or itemid == "stoleniphone" or itemid ==
        "stolens8" or itemid == "stolennokia" or itemid == "stolenpixel3" or
        itemid == "assphone" or itemid == "boomerphone") then
        TriggerEvent("phoneGui")
    end

    if (itemid == "burnerphone") then TriggerEvent('burner:ui') end

    -- if (itemid == "pdbadge") then
    --    RPC.execute("drp-gov:police:showBadge", json.decode(ItemInfo.information))
    -- end

    if (itemid == "joint" or itemid == "weed5oz" or itemid == "weedq" or itemid ==
        "beer" or itemid == "vodka" or itemid == "whiskey" or itemid == "lsdtab" or
        itemid == 'winemilkshake' or itemid == 'honestwineglass' or itemid ==
        "customjointitem") then drugitem = true end
    ...
    if (itemid == "tablet") then TriggerEvent("shithappensg") end
    ...
    if (itemid == "evidencebag") then
        ...
        if data == '{}' then
            ...
            TriggerEvent("inventory:updateItem", itemid, slot,
                         '{"used": "true"}')
            --
        else
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["drp-taskbar"]:taskBar(3000,
                                                        "Placing LSD Strip on 👅",
                                                        false, false, playerVeh)
        if (finished == 100) then
            TriggerEvent("Evidence:StateSet", 2, 1200)
            TriggerEvent("Evidence:StateSet", 24, 1200)
            TriggerEvent("fx:run", "lsd", 180, -1,
                         (itemid == "badlsdtab" and true or false))
            remove = true
        end
    end

    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid ==
        "decrypterenzo") then
        if (#(GetEntityCoords(player) - vector3(1275.49, -1710.39, 54.78)) < 3.0) then
            local finished = exports["drp-taskbar"]:taskBar(25000,
                                                            "Decrypting Data",
                                                            false, false,
                                                            playerVeh)
            if (finished == 100) then
    ...
    if (itemid == "weedoz" and hasEnoughOfItem("qualityscales", 1, false)) then
        ...
        local finished = exports["drp-taskbar"]:taskBar(5000, "Rolling Joints",
                                                        false, false, playerVeh)
        if (finished == 100) then CreateCraftOption("joint", 10, true) end

    end

    if (itemid == "weedq" and hasEnoughOfItem("qualityscales", 1, false)) then
        local finished = exports["drp-taskbar"]:taskBar(1000, "Rolling Joints",
                                                        false, false, playerVeh)
        if (finished == 100) then CreateCraftOption("joint2", 3, true) end
    end

    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation", "lighter")
        local finished = exports["drp-taskbar"]:taskBar(2000, "Starting Fire",
                                                        false, false, playerVeh)
        if (finished == 100) then end
    end

    if (itemid == "joint" or itemid == "joint2") then
        local finished = exports["drp-taskbar"]:taskBar(2000, "Smoking Joint",
                                                        false, false, playerVeh)
        if (finished == 100) then
            ...
            TriggerEvent("weed", 5000, "WORLD_HUMAN_SMOKING_POT",
                         itemid == "joint" and 1.5 or 0.75)
            remove = true
        end

        local finished = exports["drp-taskbar"]:taskBar(5000, "Armor", true,
                                                        false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "vodka" or itemid == "beer" or itemid == "whiskey" or itemid ==
        "absinthe" or itemid == "drink1" or itemid == "drink2" or itemid ==
        "drink3" or itemid == "drink4" or itemid == "drink5" or itemid ==
        "drink6" or itemid == "drink7" or itemid == "drink8" or itemid ==
        "drink9" or itemid == "drink10" or itemid == "shot1" or itemid ==
        "shot2" or itemid == "shot3" or itemid == "shot4" or itemid == "shot5" or
        itemid == "shot6" or itemid == "winemilkshake" or itemid == "shot7" or
        itemid == "shot8" or itemid == "shot9" or itemid == "shot10" or itemid ==
        "moonshine" or itemid == "poisonedcocktail" or itemid == "redwineglass" or
        itemid == "whitewineglass" or itemid == "mead_watermelon" or itemid ==
        "mead_strawberry" or itemid == "mead_potato" or itemid == "mead_peach" or
        itemid == "mead_orange" or itemid == "mead_lime" or itemid ==
        "mead_lemon" or itemid == "mead_kiwi" or itemid == "mead_grape" or
        itemid == "mead_coconut" or itemid == "mead_cherry" or itemid ==
        "mead_banana" or itemid == "mead_apple") then
        local success = itemid == "winemilkshake" and true or
                            (AttachPropAndPlayAnimation(
                                "amb@world_human_drinking@coffee@male@idle_a",
                                "idle_c", 49, 6000, "Drink", "changethirst",
                                true, itemid, playerVeh))
        if success then
            TriggerEvent("Evidence:StateSet", 8, 600)
            local alcoholStrength = 0.5
            if itemid == "vodka" or itemid == "whiskey" or itemid == "yamazaki" or
                itemid == "shochu" then alcoholStrength = 1.0 end
            if itemid == "absinthe" then alcoholStrength = 2.5 end
            if itemid == "moonshine" then alcoholStrength = 4.0 end
            if itemid == "drink1" or itemid == "drink2" or itemid == "drink3" or
                itemid == "drink4" or itemid == "drink5" or itemid == "drink6" or
                itemid == "drink7" or itemid == "drink8" or itemid == "drink9" or
                itemid == "drink10" then alcoholStrength = 0.6 end
            if itemid == "shot1" or itemid == "shot2" or itemid == "shot3" or
                itemid == "shot4" or itemid == "shot5" or itemid == "shot6" or
                itemid == "shot7" or itemid == "shot8" or itemid == "shot9" or
                itemid == "shot10" or itemid == "redwineglass" or itemid ==
                "whitewineglass" then alcoholStrength = 0.6 end
            if itemid == "mead_watermelon" or itemid == "mead_strawberry" or
                itemid == "mead_potato" or itemid == "mead_peach" or itemid ==
                "mead_orange" or itemid == "mead_lime" or itemid == "mead_lemon" or
                itemid == "mead_kiwi" or itemid == "mead_grape" or itemid ==
                "mead_coconut" or itemid == "mead_cherry" or itemid ==
                "mead_banana" or itemid == "mead_apple" then
                alcoholStrength = 1.0
            ...
            TriggerEvent("fx:run", "alcohol", 180, alcoholStrength, -1,
                         (itemid == "absinthe" and true or false))
        end
    end

    if (itemid == "coffee" or itemid == "frappuccino" or itemid == "latte" or
        itemid == "customcoffeeitem") then
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49, 6000,
            "Drink", "coffee:drink", not customMarketItems[itemid],
            itemid == "customcoffeeitem" and "coffee" or itemid, playerVeh)
        remove = customMarketItems[itemid]
    end

    if (itemid == "fishtaco") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                   "mp_player_int_eat_burger", 49, 6000,
                                   "Eating", "food:FishTaco", true, itemid,
                                   playerVeh)
    end
    ...
    if (itemid == "taco" or itemid == "burrito") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                   "mp_player_int_eat_burger", 49, 6000,
                                   "Eating", "food:Taco", true, itemid,
                                   playerVeh)
    end

    if (itemid == "churro" or itemid == "hotdog" or itemid == "chocobar" or
        itemid == "katsucurry" or itemid == "misosoup" or itemid ==
        "teriyakichicken" or itemid == "boabun" or itemid == "chickennoodles") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,
                 6000, "Eating", "food:Condiment", true, itemid, playerVeh)
    end
    ...
    if (itemid == "greencow" or itemid == "franksmonster") then
        ...
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,
            fmTimer, fmMsg, "food:Condiment", true, itemid, playerVeh)
    end

    if (itemid == "donut" or itemid == "applepie" or itemid == "eggsbacon" or
        itemid == "cookie" or itemid == "muffin") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                   "mp_player_int_eat_burger", 49, 6000,
                                   "Eating", "food:Condiment", true, itemid,
                                   playerVeh)
    end

    if (itemid == "icecream" or itemid == "mshake" or itemid == "winemilkshake" or
        itemid == "softdrink") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,
                 6000, "Eating", "inv:fed2", true, itemid, playerVeh)
    end
    ...
    if (itemid == "advlockpick") then
        ...
        if myJob ~= "news" then
            ...
            TriggerEvent("DoLongHudText",
                         "Nice news reporting, you shit lord idiot.")
        end
    ...
    if (itemid == "heistlaptop2") then TriggerEvent("drp-paleto:distcheck") end
    ...
    if (itemid == "miningpickaxe") then TriggerEvent("drp-start-mining") end

    if (itemid == "heistlaptop3") then TriggerEvent("drp-fleeca:distcheck") end
    ...
    if (itemid == "Gruppe6Card") then
        ...
        if police >= 0 then
            ...
            if targetVehicle ~= 0 and GetHashKey("stockade") ==
                ...
                local entityCreatePoint =
                    GetOffsetFromEntityInWorldCoords(targetVehicle, 0.0, -4.0,
                                                     0.0)
                ...
                if aDist < 2.0 then
                    ...
                    local finished = exports["drp-taskbar"]:taskBar(timeToOpen,
                                                                    "Unlocking Vehicle",
                                                                    false,
                                                                    false,
                                                                    playerVeh)
    ...
    if (itemid == "huntingbait") then TriggerEvent('drp-hunting:usedBait') end

    if (itemid == "huntingknife") then TriggerEvent('drp-hunting:skinAnimal') end
    ...
    if (itemid == "heavyammo") then
        local finished = exports["drp-taskbar"]:taskBar(5000, "Reloading",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pistolammo") then
        local finished = exports["drp-taskbar"]:taskBar(2500, "Reloading",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "flamethrowerammo") then
        local finished = exports["drp-taskbar"]:taskBar(20000,
                                                        "Reloading flamethrower",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "huntingammo") then
        local finished = exports["drp-taskbar"]:taskBar(10000, "Reloading",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "rpgammo") then
        local finished = exports["drp-taskbar"]:taskBar(25000, "Reloading",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "taserammo") then
        local finished = exports["drp-taskbar"]:taskBar(2000, "Reloading",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "empammo") then
        local finished = exports["drp-taskbar"]:taskBar(30000, "Recharging EMP",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["drp-taskbar"]:taskBar(10000, "Opening bag",
                                                        true, false, playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "newaccountbox") then
        local finished = exports["drp-taskbar"]:taskBar(5000, "Opening present",
                                                        false, false, playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
    ...
    if (itemid == "foodsupplycrate") then
        TriggerEvent("DoLongHudText",
                     "Make sure you have a ton of space in your inventory! 100 or more.",
                     2)
        local finished = exports["drp-taskbar"]:taskBar(20000,
                                                        "Opening the crate (ESC to Cancel)",
                                                        false, false, playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "fishingtacklebox") then
        local finished = exports["drp-taskbar"]:taskBar(5000, "Opening", true,
                                                        false, playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
    ...
    if (itemid == "housesafe") then
        if not hasEnoughOfItem("heavydutydrill", 1, false) then
            TriggerEvent("DoLongHudText",
                         "Seems like you need something to get into this...", 2)
        else
            local finished = exports["drp-taskbar"]:taskBar(30000,
                                                            "Cracking Safe...",
                                                            true, false)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "pixellaptop") then
        local finished = exports["drp-taskbar"]:taskBar(3500, "Powering UP",
                                                        false, false, playerVeh)
        if (finished == 100) then TriggerEvent("drp-boosting:openLaptop") end
    end
    ...
    if (itemid == "durpman") then
        local finished = exports["drp-taskbar"]:taskBar(1500, "Opening", false,
                                                        false, playerVeh)
        if (finished == 100) then TriggerEvent("attemt:play:song") end
    end

    if (itemid == "disabler") then
        local finished = exports["drp-taskbar"]:taskBar(500, "Powering UP",
                                                        false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "fishingchest") then
        local finished = exports["drp-taskbar"]:taskBar(5000, "Opening Chest",
                                                        true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
    ...
    if (itemid == "repcrate") then
        local finished = exports["drp-taskbar"]:taskBar(30000, "Opening...",
                                                        true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
    ...
    if (itemid == "fishinglockbox") then
        ...
        if (finished == 100) then
            ...
            -- TriggerServerEvent('loot:useItem', itemid)
            TriggerEvent("DoLongHudText",
                         "Add your map thing here DW you fucking fuck fuck", 2)
        end
    end

    if (itemid == "organcooler") then
        local finished = exports["drp-taskbar"]:taskBar(5000, "Opening cooler",
                                                        true, false, playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if itemid == "humanhead" then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,
                 10000, "Eating (ESC to Cancel)", "inv:wellfed", true, itemid,
                 playerVeh, true, "humanskull")
    end

    if (itemid == "humantorso" or itemid == "humanarm" or itemid == "humanhand" or
        itemid == "humanleg" or itemid == "humanfinger") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,
                 10000, "Eating (ESC to Cancel)", "inv:wellfed", true, itemid,
                 playerVeh, true, "humanbone")
    end

    if (itemid == "humanear" or itemid == "humanintestines" or itemid ==
        "humanheart" or itemid == "humaneye" or itemid == "humanbrain" or itemid ==
        "humankidney" or itemid == "humanliver" or itemid == "humanlungs" or
        itemid == "humantongue" or itemid == "humanpancreas") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,
                 10000, "Eating (ESC to Cancel)", "inv:wellfed", true, itemid)
    end
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem", 1, false)) then
            ...
            TriggerEvent("DoLongHudText",
                         "You are missing something to open the box with", 2)
        end
    end
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey", 1, false)) then
            ...
            TriggerEvent("DoLongHudText",
                         "You are missing something to open the briefcase with",
                         2)
        end
    end
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557", 1, false)) then
            local finished = exports["drp-taskbar"]:taskBar(15000,
                                                            "Opening supply crate.",
                                                            false, false,
                                                            playerVeh)
            if (finished == 100) then
                remove = true
            ...
            TriggerEvent("DoLongHudText",
                         "You are missing something to open the crate with", 2)
        end
    end

    if (itemid == "xmasgiftcoal") then
        local finished = exports["drp-taskbar"]:taskBar(15000, "Opening Gift",
                                                        false, false, playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
    ...
    if (itemid == "Smallsupplycrate") then
        if (hasEnoughOfItem("2227010557", 1, false)) then
            ...
            TriggerEvent("DoLongHudText",
                         "What, are you going to break it open with your hands?",
                         2)
        end
    end
    ...
    if (itemid == "emptyballoon") then
        if (hasEnoughOfItem("nitrous", 1, false)) then
            ...
            TriggerEvent("DoLongHudText",
                         "You need a bottle to fill the balloon with.", 2)
        end
    end
                ...
                Citizen.CreateThread(function()
                    while animationplaying do
                        if not IsEntityPlayingAnim(PlayerPedId(),
                                                   "misscarsteal2peeing",
                                                   "peeing_loop", 3) then
                            TaskPlayAnim(PlayerPedId(), "misscarsteal2peeing",
                                         "peeing_loop", 20.0, -8, -1, 49, 0,
                                         false, false, false)
                        end
                        Citizen.Wait(100)
                    end
    ...
    if itemid == "Peecup_full" then
        local success = AttachPropAndPlayAnimation(
                            ...
                            "idle_e", 49, 7500, "Drinking pee.",
                            "food:SoftDrink", true, itemid, playerVeh)
        if success and math.random() > 0.50 then
            TriggerServerEvent("fx:puke")
            TriggerEvent("animation:PlayAnimation", "outofbreath")
            TaskPlayAnim(PlayerPedId(), "re@construction", "out_of_breath",
                         20.0, -8, -1, 49, 0, false, false, false)
            Citizen.Wait(6000)
            TriggerEvent("animation:PlayAnimation", "cancel")
        end
    end

    if (itemid == "binoculars") then TriggerEvent("binoculars:Activate") end

    if (itemid == "camera") then TriggerEvent("camera:Activate") end

    if (itemid == "megaphone") then TriggerEvent("drp-usableprops:megaphone") end
    ...
    if (itemid == "nitrous") then
        ...
        if currentVehicle == nil or currentVehicle == 0 then
            ...
            TaskItem("amb@code_human_wander_idles@male@idle_a",
                     "idle_b_rubnose", 49, 2800, "Sniff Sniff", "hadnitrous",
                     false, itemid, playerVeh)
        ...
        elseif not IsToggleModOn(currentVehicle, 18) then
            ...
            if (finished == 100 and not cancelNos) then
                ...
                TriggerEvent("DoLongHudText",
                             "You can't drive and hook up nos at the same time.",
                             2)
            end
        end
    end
    ...
    if (itemid == "godlockpick") then
        local myJob = exports["isPed"]:isPed("myJob")
        if myJob ~= "news" then
            TriggerEvent("inv:lockPick2", false, inventoryName, slot,
                         "godlockpick")
    ...
    if (itemid == "securityblue" or itemid == "securityblack" or itemid ==
        "securitygreen" or itemid == "securitygold" or itemid == "securityred") then
    ...
    if (itemid == "ciggy") then
        local finished = exports["drp-taskbar"]:taskBar(1000, "Lighting Up",
                                                        false, false, playerVeh)
        if (finished == 100) then
            Wait(300)
    ...
    if (itemid == "oxygentank") then
        local finished = exports["drp-taskbar"]:taskBar(30000, "Oxygen Tank",
                                                        true, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bandage" or itemid == "custombandageitem") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49, 10000,
                 "Healing", "healed:minors", true, itemid, playerVeh)
    end

    if (itemid == "bandagelarge") then
        TriggerEvent("inventory:DegenLastUsedItem", 33)
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49, 10000,
                 "Healing", "healed:minors", false, itemid, playerVeh)
        Citizen.CreateThread(function()
            Wait(32000)
    ...
    if (itemid == "bakingsoda") then CreateCraftOption("1gcrack", 80, true) end

    if (itemid == "glucose") then CreateCraftOption("1gcocaine", 80, true) end
    ...
    if (itemid == "adrenaline") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49, 1000,
                 "POG GAMING Adrenaline", "inventory:adrenaline", true, itemid,
                 playerVeh)
    end
    ...
    if (itemid == "Desomorphine") then
        local success = TaskItem("amb@world_human_clipboard@male@idle_a",
                                 "idle_c", 49, 5000, "Injecting Desomorphine",
                                 "inventory:adrenaline", true, itemid, playerVeh)
        if success then
            ...
            SetPedMovementClipset(PlayerPedId(), "move_m@drunk@slightlydrunk",
                                  1.0)

            local ufoModelHash = GetHashKey("dt1_tc_dufo_core_np")
            RequestModel(ufoModelHash)

            while not HasModelLoaded(ufoModelHash) do Citizen.Wait(1) end

            local coords = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 25.0,
                                                            25.0, 100.0)
            local ufoObject = CreateObject(ufoModelHash, coords, false, false,
                                           false)
            ...
            Citizen.CreateThread(function()
                while drugEffect do
                    ...
                    if math.random() < 0.11 then
                        TaskVehicleTempAction(PlayerPedId(), curVeh,
                                              randomDrivingActions[math.random(
                                                  #randomDrivingActions)], 4000)
                    end
                    SetEntityHealth(PlayerPedId(),
                                    GetEntityHealth(PlayerPedId()) - 1)
                    if GetEntityHealth(PlayerPedId()) < 105 then
                        ...
                        if math.random() < 0.70 then
                            ...
                            TriggerEvent("animation:PlayAnimation",
                                         "outofbreath")
                            TaskPlayAnim(PlayerPedId(), "re@construction",
                                         "out_of_breath", 20.0, -8, -1, 49, 0,
                                         false, false, false)
                            Citizen.Wait(6000)
                            TriggerEvent("animation:PlayAnimation", "cancel")
                        end
                        ...
                        if math.random() < 0.25 or WillOD >= 3 then
                            SetEntityHealth(PlayerPedId(), GetEntityHealth(
                                                PlayerPedId()) - 100)
                            WillOD = 0
                        else
    ...
    if (itemid == "1gcocaine") then
        ...
        TaskItem("anim@amb@nightclub@peds@",
                 "missfbi3_party_snort_coke_b_male3", 49, 5000, "Coke Gaming",
                 "hadcocaine", true, itemid, playerVeh)
    end
    ...
    if (itemid == "1gcrack") then
        ...
        TriggerEvent("Evidence:StateSet", 2, 1200)
        TriggerEvent("Evidence:StateSet", 6, 1200)
        TaskItem("switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop",
                 49, 5000, "Smoking Quack", "hadcrack", true, itemid, playerVeh)
    end
    ...
    if (itemid == "nosballoon") then
        ...
        TriggerEvent("Evidence:StateSet", 6, 1200)
        TaskItem("switch@trevor@trev_smoking_meth", "trev_smoking_meth_loop",
                 49, 5000, "Huffing Nos", "hadcrack", true, itemid, playerVeh)
    end
    ...
    if (itemid == "treat") then
        ...
        if validDogModels[model] then
            TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,
                     1200, "Treat Num's", "hadtreat", true, itemid, playerVeh)
        end
    end

    if (itemid == "IFAK") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49, 2000,
                 "Applying IFAK", "healed:useOxy", true, itemid, playerVeh)
    end

    if (has_value(fruits, itemid)) then
        AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                   "mp_player_int_eat_burger", 49, 6000,
                                   "Eating", true, itemid, playerVeh)
    ...
    if (itemid == "experiment_01" or itemid == "experiment_02" or itemid ==
        "experiment_03" or itemid == "experiment_04" or itemid ==
        "experiment_05" or itemid == "experiment_06" or itemid ==
        "experiment_07" or itemid == "experiment_08" or itemid ==
        "experiment_09") then
        TriggerEvent("animation:PlayAnimation", "pill")
        local finished = exports["drp-taskbar"]:taskBar(3000,
                                                        "Downing content of bottle. 💊",
                                                        false, false, playerVeh)
        if (finished == 100) then
            ...
            if math.random() < 0.50 then
                TriggerEvent("fx:run",
                             randomEffects[math.random(#randomEffects)], 280,
                             -1, false)
            end
            remove = true
        end
    ...
    if itemid == "copium" then
        local success = TaskItem("switch@trevor@trev_smoking_meth",
                                 "trev_smoking_meth_loop", 49, 10000,
                                 "Taking a hit of copium", "hadtreat", true,
                                 itemid, playerVeh)
        if success then
    ...
    if sandwichItems[itemid] or deanworldFood[itemid] then
        AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                   "mp_player_int_eat_burger", 49, 6000,
                                   "Eating", not customMarketItems[itemid],
                                   itemid, playerVeh)
        remove = true
    ...
    if deanworldFood[itemid] then
        ...
        if math.random() < 0.05 then TriggerServerEvent("fx:puke") end
    end
    ...
    if waterItems[itemid] then
        ...
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@beer@female@idle_a", "idle_e", 49, 6000,
            "Drink", not customMarketItems[itemid], itemid, playerVeh)
    ...
    if itemid == "kdragonwater" then
        ...
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@beer@female@idle_a", "idle_e", 49, 6000,
            "Drink", "drp-katana:transformKFromDrink", true, itemid, playerVeh)
        remove = true
    end

    if (itemid == "bleederburger" or itemid == "heartstopper" or itemid ==
        "torpedo" or itemid == "meatfree" or itemid == "moneyshot" or itemid ==
        "maccheese" or itemid == "questionablemeatburger" or itemid == "panini" or
        itemid == "pizza" or itemid == "pancakes" or itemid == "wings") then
        AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                   "mp_player_int_eat_burger", 49, 6000,
                                   "Eating", "inv:wellfed", true, itemid,
                                   playerVeh)
        -- attachPropsToAnimation(itemid, 6000)
        -- TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
    end

    if (itemid == "sushiplate" or itemid == "sushiplatec" or itemid ==
        "beefdish" or itemid == "beefdishc" or itemid == "ramen") then
        local success = AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                                   "mp_player_int_eat_burger",
                                                   49, 6000, "Eating",
                                                   "inv:wellfedNoStress", true,
                                                   itemid, playerVeh)
        if success then
    ...
    if (itemid == "blue_rare_steak" or itemid == "rare_steak" or itemid ==
        "medium_rare_steak" or itemid == "medium_steak" or itemid ==
        "medium_well_steak" or itemid == "well_done_steak") then
        local success = AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                                   "mp_player_int_eat_burger",
                                                   49, 6000, "Eating",
                                                   "inv:wellfedNoStress", true,
    ...
    if (itemid == "methlabproduct") then
        local finished = exports["drp-ui"]:taskBarSkill(2400,
                                                        math.random(12, 18))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim", "crackpipe01")
            TriggerEvent("Evidence:StateSet", 2, 1200)
            ...
            TaskItem("switch@trevor@trev_smoking_meth",
                     "trev_smoking_meth_loop", 49, 1500, "Smoking Meth",
                     "hadmeth", true, itemid, playerVeh, nil, nil,
    ...
    if itemid == "ketamine" then
        local finished = exports["drp-ui"]:taskBarSkill(2000,
                                                        math.random(10, 15))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim", "drugpackage01")
            TriggerEvent("Evidence:StateSet", 2, 1200)
            TriggerEvent("Evidence:StateSet", 6, 1200)
            TaskItem("anim@amb@nightclub@peds@",
                     "missfbi3_party_snort_coke_b_male3", 49, 5000,
                     "Entering the K-Hole", "inventory:ketamine", true, itemid,
                     playerVeh, nil, nil, json.decode(passedItemInfo).purity)
        end
    end
    ...
    if itemid == "slushy" then
        ...
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@beer@female@idle_a", "idle_e", 49, 6000,
            "Eating", "inv:slushy", true, itemid, playerVeh)
    end
    ...
    if itemid == "asahi" then
        ...
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@beer@female@idle_a", "idle_e", 49, 6000,
            "Eating", "inv:well", true, itemid, playerVeh)
    end

    if itemid == "fruitslushy" then
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@beer@female@idle_a", "idle_e", 49, 6000,
            "Drinking", "inv:fruitslushy", true, itemid, playerVeh)
    end
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished =
            exports["drp-ui"]:taskBarSkill(1000, math.random(7, 12))
        if (finished == 100) then TriggerEvent("police:uncuffMenu") end
        lockpicking = false
        remove = true
    end

    if (itemid == "watch") then TriggerEvent("drp-ui:watch") end

    if (itemid == "tuner") then
        local finished = exports["drp-taskbar"]:taskBar(2000,
                                                        "Connecting Tuner Laptop",
                                                        false, false, playerVeh)
        if (finished == 100) then TriggerEvent("tuner:open") end
    end

    if (itemid == "godbook") then
        local ItemInfo = GetItemInfo(slot)
        if (ItemInfo == nil or ItemInfo.information == "No information stored" or
            ItemInfo.information == nil) then
            justUsed = false
            retardCounter = 0
            lastCounter = 0
            return
    ...
    if itemid == "softdrink" then
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49, 15000,
            "Drink", "food:SoftDrink", true, itemid, playerVeh)
    end

    if itemid == "roostertea" then
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49, 15000,
            "Drink", "drp-roostersrest:drinkTea", true, itemid, playerVeh)
    end

    if itemid == "fries" or itemid == "chips" then
        AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                   "mp_player_int_eat_burger", 49, 15000,
                                   "Eating", "inv:fed2", true, itemid, playerVeh)
    end

    if itemid == "bscoffee" then
        AttachPropAndPlayAnimation(
            "amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49, 15000,
            "Drink", "coffee:drink", true, itemid, playerVeh)
    end

    if itemid == "foodbag" or itemid == "collectiblepouch" then
        local data = exports["isPed"]:isPed("cid")
        -- local data = json.decode(passedItemInfo)
        TriggerEvent("inventory-open-container", data.inventoryId, data.slots,
                     data.weight)
    end

    if itemid == "randomtoy" then TriggerEvent("murdermeal:toys1") end

    if itemid == "murdermeal" then TriggerEvent("murdermeal", ItemInfo.id) end

    if itemid == "pdevidencebag" then
        TriggerEvent("pdevidencebag", ItemInfo.id)
    end

    if itemid == "keyholder" then TriggerEvent("keyholder") end
    ...
    if itemid == "franksflute" then
        TriggerServerEvent("drp-inventory:franksFlute",
                           GetEntityCoords(PlayerPedId()))
    ...
    if itemid == "tattooremover" then
        ...
        if IsEntityAPed(target) and IsPedAPlayer(target) then
            TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_WELDING", 0,
                                     true)
            local finished = exports["drp-taskbar"]:taskBar(60000,
                                                            "Removing tattoos")
            ...
            if finished == 100 then
                ...
                if #(plyCoords - targetCoords) < 5.0 then
                    local plyId = GetPlayerServerId(
                                      NetworkGetPlayerIndexFromPed(target))
                    local success = RPC.execute(
                                        "clothing:removeTattoosForPlayer", plyId)
                    if success then
    ...
    if itemid == "buddhaletter" then
        local finished = exports["drp-taskbar"]:taskBar(5000, "Opening Parcel.",
                                                        false, false, playerVeh)
        if finished == 100 then
            TriggerEvent("DoLongHudText",
                         "As you open the letter, you feel a great sense of power.",
                         10)
            remove = true
    ...
    if itemid == "buddhablade" or itemid == "buddhaguard" or itemid ==
        "buddhahilt" then
        if hasEnoughOfItem("buddhablade", 1, false) and
            hasEnoughOfItem("buddhaguard", 1, false) and
            hasEnoughOfItem("buddhahilt", 1, false) then
            if hasEnoughOfItem("buddhamedalion", 1, false) then
                local playerPos = GetEntityCoords(PlayerPedId())
                if #(playerPos - vector3(-453.333, 1.6416, 35.5193)) < 3 then
                    local finished = exports["drp-taskbar"]:taskBar(30000,
                                                                    "Putting blade together.",
                                                                    false,
                                                                    false,
                                                                    playerVeh)
                    if (finished == 100) then
                    ...
                    TriggerEvent("DoLongHudText",
                                 "Voice in your head: The parts are calling to the tunnels",
                                 5)
                end
            else
                TriggerEvent("DoLongHudText",
                             "You seem to be missing something.", 5)
            end
        else
            TriggerEvent("DoLongHudText",
                         "Voice in your head: Your quest is not yet complete.",
                         5)
        end
    end
    if itemid == "buddhashovel" then
        ...
        if #(playerPos - vector3(-1911.78, 1388.1, 219.35)) < 10 then
            if BuddhaIsItemFound1 == false then
                TaskStartScenarioInPlace(PlayerPedId(),
                                         "WORLD_HUMAN_GARDENER_PLANT", 0, true)
                local finished = exports["drp-taskbar"]:taskBar(15000,
        ...
        elseif not BuddhaIsItemFound1 or not BuddhaIsItemFound2 or
            not BuddhaIsItemFound3 then
            TriggerEvent("DoLongHudText",
                         "Voice in your head: This is not the place.", 5)
        else
            TriggerEvent("DoLongHudText",
                         "Voice in your head: You already found it, move on!", 5)
        end
    end

    if itemid == "notepad" then notepadUsed() end
    if itemid == "notepadnote" then notepadPageUsed(passedItemInfo) end

    TriggerEvent("drp-inventory:itemUsed", itemid, passedItemInfo,
                 inventoryName, slot)
    TriggerServerEvent("drp-inventory:itemUsed", itemid, passedItemInfo,
                       inventoryName, slot)

    if remove then
        local info = json.decode(passedItemInfo)
        if info and info._remove_id then
            TriggerEvent("inventory:removeItemByMetaKV", itemid, 1,
                         "_remove_id", info._remove_id)
        else
            TriggerEvent("inventory:removeItem", itemid, 1)
        end
    end
...
function AttachPropAndPlayAnimation(dictionary, animation, typeAnim, timer,
                                    message, func, remove, itemid, vehicle)
    if itemid == "hamburger" or itemid == "heartstopper" or itemid ==
        "bleederburger" or itemid == "moneyshot" or itemid == "torpedo" or
        itemid == "questionablemeatburger" then
    ...
    elseif waterItemsAttached[itemid] then
        TriggerEvent("attachItem",
                     (itemid == "poisonedwater" or itemid == "kdragonwater" or
                         itemid == "customwateritem") and "water" or itemid)
    elseif itemid == "drink1" or itemid == "drink2" or itemid == "drink3" or
        itemid == "drink4" or itemid == "drink5" or itemid == "drink6" or itemid ==
        "drink7" or itemid == "drink8" or itemid == "drink9" or itemid ==
        "drink10" or itemid == "absinthe" or itemid == "moonshine" then
        TriggerEvent("attachItem", "whiskeyglass")
    elseif itemid == "shot1" or itemid == "shot2" or itemid == "shot3" or itemid ==
        "shot4" or itemid == "shot5" or itemid == "shot6" or itemid == "shot7" or
        itemid == "shot8" or itemid == "shot9" or itemid == "shot10" then
    ...
    elseif itemid == "greencow" or itemid == "franksmonster" or itemid ==
        "chuhai" then
        TriggerEvent("attachItem", "energydrink")
    elseif itemid == "slushy" then
        TriggerEvent("attachItem", "cup")
    elseif itemid == "blue_rare_steak" or itemid == "rare_steak" or itemid ==
        "medium_rare_steak" or itemid == "medium_steak" or itemid ==
        "medium_well_steak" or itemid == "well_done_steak" then
    ...
    local success = TaskItem(dictionary, animation, typeAnim, timer, message,
                             func, remove, itemid, vehicle)
    TriggerEvent("destroyProp")
    return success
end
...
AddEventHandler("drp-inventory:attachPropPlayAnim", function(pType)
    ...
    if pType == "sandwich" then
        success = AttachPropAndPlayAnimation("mp_player_inteat@burger",
                                             "mp_player_int_eat_burger", 49,
                                             6000, "Eating", "", false,
                                             "sandwich", vehicle)
    end
    if pType == "fries" then
...
AddEventHandler('randPickupAnim', function()
    ...
    TaskPlayAnim(PlayerPedId(), 'pickup_object', 'putdown_low', 5.0, 1.5, 1.0,
                 48, 0.0, 0, 0, 0)
    Wait(1000)
...
function GetItemName(itemid)
    for i, v in pairs(clientInventory) do
        if (v.item_id == itemid) then return v.name end
    end
...
AddEventHandler('drp-business:repairItem', function()
    ...
    local itemListInfo = json.decode(exports["drp-inventory"]:itemListInfo(
                                         info.item_id))
    local allowed = true
    ...
    local failed =
        ((info.amount > 1 and "You can only repair 1 item at a time") or
            (not QualityCheck and "Can only repair items between 0-80%. ") or
            (tonumber(info.item_id) ~= nil and "Can not repair weapons. ") or
            (not itemListInfo and "Item is unrepairable. ") or
            (not allowed and "Not enough materials. "))
...
AddEventHandler('drp-business:examineItem', function()
    ...
    local failed =
        ((info.amount > 1 and "You can only repair 1 item at a time") or
            (not QualityCheck and "Can only repair items between 0-80%. ") or
            (tonumber(info.item_id) ~= nil and "Can not repair weapons. ") or
            (not itemListInfo and "Item is unrepairable. "))
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0},
                 " ^1Item current quality is: " .. Quality ..
                     " we will repair a total of %" .. PercentRepair);
...
function TaskItem(dictionary, animation, typeAnim, timer, message, func, remove,
                  itemid, playerVeh, itemreturn, itemreturnid, quality)
    loadAnimDict(dictionary)
    TaskPlayAnim(PlayerPedId(), dictionary, animation, 8.0, 1.0, -1, typeAnim,
                 0, 0, 0, 0)
    local timer = tonumber(timer)
    if timer > 0 then
        local finished = exports["drp-taskbar"]:taskBar(timer, message, true,
                                                        false, playerVeh)
...
function getFreeSpace()
    local space = 40
    for i, v in pairs(clientInventory) do
        if v.item_id then space = space - 1 end
    end
    return space
end
...
function hasEnoughOfItem(itemid, amount, shouldReturnText, checkQuality,
    ...
    if itemid == nil or itemid == 0 or amount == nil or amount == 0 then
        if shouldReturnText then
            TriggerEvent("DoLongHudText",
                         "I dont seem to have " .. itemid .. " in my pockets.",
                         2)
        end
        return false
    end
...
function getItemsOfType(itemid, limitAmount, checkQuality, metaInformation)
    ...
    for i, v in pairs(clientInventory) do
        if amount and #itemsFound >= amount then break end
...
AddEventHandler('inventory:DegenItemType', function(percent, itemid)
    ...
    print(
        "Degen applied to " .. itemid .. " ID: " .. itemFound[1].id .. " at %" ..
            percent)
    TriggerServerEvent("inventory:degItem", itemFound[1].id, percent, itemid,
                       cid)
end)

function checkForAttachItem()
    local AttatchItems = {
        "stolentv", "stolenmusic", "stolencoffee", "stolenmicrowave",
        "stolencomputer", "stolenart", "darkmarketpackage", "weedpackage",
        "methpackage", "boxscraps", "dodopackagesmall", "dodopackagemedium",
        "dodopackagelarge", "housesafe", "fridge"
    }
...
function checkForItems()
    local items = {
        "mobilephone", "stoleniphone", "stolennokia", "stolenpixel3",
        "stolens8", "boomerphone", "radio", "civradio", "burgershotheadset",
        "megaphone", "watch", "boxinggloves", "cgchain", "gsfchain",
        "cerberuschain", "mdmchain", "vagoschain", "koilchain", "racingusb"
    }
...
function isValidUseCase(itemID, isWeapon)
    ...
    if playerVeh ~= 0 then
        ...
        if IsThisModelACar(model) or IsThisModelABike(model) or
            IsThisModelAQuadbike(model) then
            if IsEntityInAir(playerVeh) then
                Wait(1000)
                if IsEntityInAir(playerVeh) then
                    TriggerEvent("DoLongHudText",
                                 "You appear to be flying through the air", 2)
                    return false
                end
            end
        end
    ...
    if not validWaterItem[itemID] and not isWeapon then
        if IsPedSwimming(player) then
            ...
            if #(targetCoords - plyCoords) > 1.3 then
                TriggerEvent("DoLongHudText",
                             "Cannot be moving while swimming to use this.", 2)
                return false
            end
        end
...
AddEventHandler('CheckDNA',
                function() TriggerServerEvent("Evidence:checkDna") end)

RegisterNetEvent('evidence:dnaSwab')
AddEventHandler('evidence:dnaSwab', function(pArgs, pEntity, pContext)
    TriggerServerEvent("police:dnaAsk",
...
function GetPlayers()
    ...
    for i = 0, 255 do
        if NetworkIsPlayerActive(i) then players[#players + 1] = i end
    end
...
function GetClosestPlayer()
    ...
    for index, value in ipairs(players) do
        ...
        if (target ~= ply) then
            ...
            local distance = #(vector3(targetCoords["x"], targetCoords["y"],
                                       targetCoords["z"]) -
                                 vector3(plyCoords["x"], plyCoords["y"],
                                         plyCoords["z"]))
...
function buddhaMedalion()
    ...
    if not hasGuard and dist > distToGuard then dist = distToGuard end
    local distToHilt = #(playerPos - vector3(3647.61, 3121.15, 1.31))
    if not hasHilt and dist > distToHilt then dist = distToHilt end

    local oldDist = buddhaEventLastDist
    buddhaEventLastDist = dist

    if dist < 10 then
        return TriggerEvent("DoLongHudText",
                            "The medallion is shaking uncontrollably.", 5)
    end
    if dist < 2500 then
        if dist > oldDist then
            return TriggerEvent("DoLongHudText",
                                "The humming is getting weaker.", 5)
        end
        if dist < oldDist then
            return TriggerEvent("DoLongHudText",
                                "The humming is getting stronger.", 5)
        end
...
function getVehicleInDirection(coordFrom, coordTo)
    ...
    for i = 0, 100 do
        rayHandle = CastRayPointToPoint(coordFrom.x, coordFrom.y, coordFrom.z,
                                        coordTo.x, coordTo.y,
                                        coordTo.z + offset, 2, PlayerPedId(), 0)
...
AddEventHandler('animation:lockpickinvtestoutside', function(itemType)
    ...
    if itemType == "hackingdevice" then
        ...
        while lockpicking do Wait(2500) end
        ClearPedTasks(lPed)
        return
    end
    RequestAnimDict(anim.dict)
    while not HasAnimDictLoaded(anim.dict) do Citizen.Wait(0) end
...
AddEventHandler('animation:lockpickinvtest', function(disable)
    ...
    while lockpicking do
        ...
        if not IsEntityPlayingAnim(lPed, "mini@repair", "fixing_a_player", 3) then
            ...
            TaskPlayAnim(lPed, "mini@repair", "fixing_a_player", 8.0, -8, -1,
                         16, 0, 0, 0, 0)
        end
        Citizen.Wait(1)
    end
...
AddEventHandler('inv:lockPick', function(isForced, inventoryName, slot)
    ...
    if targetVehicle == 0 then
        ...
        local leftfront = GetOffsetFromEntityInWorldCoords(targetVehicle,
                                                           d1["x"] - 0.25, 0.25,
                                                           0.0)

        local count = 5000
        local dist = #(vector3(leftfront["x"], leftfront["y"], leftfront["z"]) -
                         GetEntityCoords(PlayerPedId()))
        while dist > 2.0 and count > 0 do
            dist = #(vector3(leftfront["x"], leftfront["y"], leftfront["z"]) -
                       GetEntityCoords(PlayerPedId()))
            Citizen.Wait(1)
            count = count - 1
            DrawText3Ds(leftfront["x"], leftfront["y"], leftfront["z"],
                        "Move here to lockpick.")
        end
        ...
        if finished == 100 then
            ...
            if #(GetEntityCoords(targetVehicle) - GetEntityCoords(PlayerPedId())) <
                10.0 and targetVehicle ~= 0 and GetEntitySpeed(targetVehicle) <
                5.0 then
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local carTimer = GetVehicleHandlingFloat(targetVehicle,
                                                     'CHandlingData',
                                                     'nMonetaryValue')
            if carTimer == nil then
                carTimer = math.random(25000, 180000)
            end
            if carTimer < 25000 then carTimer = 25000 end

            if carTimer > 180000 then carTimer = 180000 end

            carTimer = math.ceil(carTimer / 3)

            local myJob = exports["isPed"]:isPed("myJob")
            if myjob == "towtruck" then carTimer = 4000 end

            TriggerEvent("civilian:alertPolice", 12.0, "lockpick", targetVehicle)
            local finished =
                exports["drp-bar"]:taskBar(5000, math.random(5, 10))
            local lockpickbreak = math.random(1, 10)
            if finished ~= 100 and lockpickbreak == 1 then

                TriggerEvent("DoLongHudText", "The lockpick bent out of shape.",
                             2)
            ...
            if finished == 100 then
                ...
                local chance = math.random(50)
                if #(GetEntityCoords(targetVehicle) -
                    GetEntityCoords(PlayerPedId())) < 10.0 and targetVehicle ~=
                    0 and GetEntitySpeed(targetVehicle) < 5.0 then
...
AddEventHandler('inv:lockPick2', function(isForced, inventoryName, slot)
    ...
    if targetVehicle == 0 then
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            if carTimer < 2500 then carTimer = 2500 end

            if carTimer > 3800 then carTimer = 3800 end

            carTimer = math.ceil(carTimer / 3)

            local myJob = exports["isPed"]:isPed("myJob")
            if myjob == "towtruck" then carTimer = 1000 end
...
AddEventHandler('veh:repairing', function(inventoryName, slot, itemid)
    ...
    if itemid == "advrepairkit" or itemid == "pdrepairkit" or itemid ==
        "racerepairkit" then advanced = true end

    if itemid == "advrepairkit" and advcount > maxadv then
        TriggerEvent("DoLongHudText",
                     "You can only use this kit " .. maxadv .. " times.", 2)
    ...
    if targetVehicle ~= 0 then
        ...
        local d1, d2 = GetModelDimensions(GetEntityModel(targetVehicle))
        local moveto = GetOffsetFromEntityInWorldCoords(targetVehicle, 0.0,
                                                        d2["y"] + 0.5, 0.2)
        local dist = #(vector3(moveto["x"], moveto["y"], moveto["z"]) -
                         GetEntityCoords(PlayerPedId()))
        local count = 1000
        local fueltankhealth = GetVehiclePetrolTankHealth(targetVehicle)

        while dist > 1.5 and count > 0 do
            dist = #(vector3(moveto["x"], moveto["y"], moveto["z"]) -
                       GetEntityCoords(PlayerPedId()))
            Citizen.Wait(1)
            count = count - 1
            DrawText3Ds(moveto["x"], moveto["y"], moveto["z"],
                        "Move here to repair.")
        end
        ...
        if dist < 1.5 then
            ...
            if advanced then
                ...
                repairlength = ((3500 -
                                   (GetVehicleEngineHealth(targetVehicle) * 3) -
                                   (GetVehicleBodyHealth(targetVehicle)) / 2) *
                                   5) + 2000
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local timer = math.ceil(
                                          GetVehicleEngineHealth(targetVehicle) *
                                              5)
                        if timer < 2000 then timer = 2000 end
                        ...
                        if finished == 100 then
                            ...
                            if math.random(100) > 55 then
                                TriggerEvent("inventory:removeItem",
                                             "repairtoolkit", 1)
                            end
...
local function repairVehiclePart(pTargetVehicle, pData)
    for part, data in pairs(pData) do
        ...
        elseif part == "Tyre" then
            for i = 0, 11 do SetVehicleTyreFixed(pTargetVehicle, i) end
        end
    end
end
...
AddEventHandler('animation:repair', function(veh)
    ...
    while fixingvehicle do
        local anim3 = IsEntityPlayingAnim(PlayerPedId(), "mini@repair",
                                          "fixing_a_player", 3)
        if not anim3 then
            TaskPlayAnim(PlayerPedId(), "mini@repair", "fixing_a_player", 8.0,
                         -8, -1, 16, 0, 0, 0, 0)
        end
        Citizen.Wait(1)
    end
...
function getInformationFromFirstItem(itemToFind)
    local foundItem = exports["drp-inventory"]:GetInfoForFirstItemOfName(
                          itemToFind)
...
--
local isDead = false
AddEventHandler("pd:deathcheck", function() isDead = not isDead end)
...
AddEventHandler("drp-inventory:itemUsed", function(name, info)
    ...
    if not potency or potency < 1 then potency = 1 end
    if potency and potency > 101 then potency = 101 end
    ...
    Citizen.CreateThread(function()
        ...
        while loopCount < duration and not isDead do
            ...
            SetEntityHealth(PlayerPedId(),
                            GetEntityHealth(PlayerPedId()) - potency)
...
function openBurner(info)
    ...
    TaskPlayAnim(PlayerPedId(), "cellphone@", "cellphone_text_read_base", 2.0,
                 3.0, -1, 49, 0, 0, 0, 0)
    TriggerEvent("attachItemPhone", "phone01")
    exports["drp-ui"]:openApplication("burner", info)
end

AddEventHandler("ammonium_bicarbonate:insert",
                function(pOriginInventory, pTargetInventory, pOriginSlot,
                         pTargetSlot, pOriginItemId, pTargetItemId,
                         pOriginItemInfo, pTargetItemInfo)
    ...
    if quantity > 5 then
        return TriggerEvent("DoLongHudText",
                            "Hmm seems like I can't do that many...", 2)
    end
    ...
    local finished = exports["drp-taskbar"]:taskBar(5000 * quantity,
                                                    "Breaking down into baggies...",
                                                    false, false)
...
function notepadUsed()
    exports["drp-ui"]:openApplication("notepad", {canSave = true})
...
RegisterUICallback('drp-ui:createNotepadNote', function(data, cb)
    ...
    TriggerEvent("player:receiveItem", "notepadnote", 1, false,
                 {["content"] = data.content, ["_hideKeys"] = {"content"}})
...
function notepadPageUsed(itemInfo)
    ...
    exports["drp-ui"]:openApplication("notepad",
                                      {canSave = false, content = json.content})
end
