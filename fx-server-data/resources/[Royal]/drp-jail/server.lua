...
RegisterCommand('unjail', function(source, args)
    ...
    local user = exports["drp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local target = exports["drp-base"]:getModule("Player"):GetUser(args[1])
    ...
    if user:getVar("job") == 'police' or user:getVar("job") == 'state' or user:getVar("job") == 'police' or user:getVar("job") == 'doc' or user:getVar("job") == 'judge' then
        if args[1] and exports["drp-base"]:getModule("Player"):GetUser(tonumber(args[1])) then
...
RegisterServerEvent('drp-jail:remove-time-electrical')
AddEventHandler("drp-jail:remove-time-electrical", function()
    local src = source
    local user = exports["drp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
