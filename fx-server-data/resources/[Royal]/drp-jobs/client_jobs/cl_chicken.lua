				...
				if (lockingAnimation == 100) then
					...
					if exports["drp-inventory"]:hasEnoughOfItem("freshmeat", 5) then
						...
						if math.random(1,6) == 1 then
							...
							TriggerEvent('player:receiveItem', "cut_chicken", math.random(2,3))	
						end
...
AddEventHandler("drp-chickens:sellprotein", function()
	...
	local finished = exports['drp-taskbar']:taskBar(5000*toSell/5, 'Selling Protein')
...
AddEventHandler("drp-chickens:sellcuttedchicken", function()
	...
	local finished = exports['drp-taskbar']:taskBar(5000*toSell/5, 'Selling cutted Chickens')
	LoadDict('amb@medic@standing@tendtodead@exit')
	TaskPlayAnim(GetPlayerPed(-1), 'amb@medic@standing@tendtodead@exit', 'exit', 8.0, -8.0, -1, 1, 0.0, 0, 0, 0)
	if (finished == 100) then		
