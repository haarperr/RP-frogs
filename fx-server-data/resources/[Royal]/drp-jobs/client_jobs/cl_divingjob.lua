...
AddEventHandler('BeginDivingjob', function()
    local timer = 5000
    local finished = exports["drp-taskbar"]:taskBar(timer,"Checking your cash")
    if finished then
        TriggerServerEvent('drp-scuba:checkAndTakeDepo')
    end
end)
...
AddEventHandler('getDiveingjob', function()
  TriggerEvent('drp-jobs:SpawnDivingBoat')
  Wait(2000)
  print('HERE')
  StartDive()
end)
                ...
                if dis <= 20 and value['active'] == 1 then
                    ...
                    if dis <= 5 then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local finished = exports["drp-taskbar"]:taskBar(waiter,"Searching For Treasure")
                            if finished == 100 then
                            value['active'] = 0
                            scavanged = scavanged + 1
                            FreezeEntityPosition(PlayerPedId(), false)
                            ClearPedTasksImmediately(PlayerPedId())
                            TriggerServerEvent('drp-scuba:findTreasure')
...
function SellItems()
    if exports["drp-inventory"]:hasEnoughOfItem("ironbar",10,false) then 
        TriggerEvent("inventory:removeItem", "ironbar", 10)
        TriggerServerEvent('drp-scuba:paySalvage', 250)
    else
        TriggerEvent('DoLongHudText', 'You dont have enough Material in your pockets to sell!', 2)
    end
end

RegisterNetEvent('drp-jobs:SpawnDivingBoat')
AddEventHandler('drp-jobs:SpawnDivingBoat', function()
	Citizen.Wait(1)
