...
RegisterNetEvent('drp-civjobs:passed')
AddEventHandler('drp-civjobs:passed', function()
    isInJobPost = true
...
RegisterNetEvent('drp-civjobs:start-post-op')
AddEventHandler('drp-civjobs:start-post-op', function()
    if isInJobPost == false then
        local finished = exports['drp-taskbar']:taskBar(7500, 'Awaiting Vehicle')
        if (finished == 100) then
            TriggerEvent('drp-civjobs:spawn-truck')
        end
    end
end)

RegisterNetEvent('drp-civjobs:post-op:paycheck')
AddEventHandler('drp-civjobs:post-op:paycheck', function()
    ...
    while true do
        ...
        if isToDropLocation == true then
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 2.5 then
                if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("BOXVILLE4")) then
                    exports['drp-textui']:showInteraction('[E] Drop Off Package')
...
RegisterNetEvent("drp-civjobs:spawn-truck")
AddEventHandler("drp-civjobs:spawn-truck", function()
	Citizen.CreateThread(function()
        ...
        TriggerEvent('drp-civjobs:passed')
