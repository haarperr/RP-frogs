        ...
        for i = 1, #Best_buds[regID] do
            if Best_buds[regID][i].regID == regID then
                ...
                if (tonumber(user:getCash()) >= tonumber(amount)) then
                    ...
                    local owner = exports["drp-base"]:getModule("Player"):GetUser(Best_buds[regID][i].owner)
                    ...
                    TriggerClientEvent("player:receiveItem", Best_buds[regID][i].owner, "burgerReceipt", 1, true, information)
