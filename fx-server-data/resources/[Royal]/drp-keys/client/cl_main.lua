...
function shutoffenginesearch()
    ...
    if not fuckingRETARDED then
       ...
       fuckingRETARDED = true
        ...
        local finished = exports["drp-taskbar"]:taskBar(5000,"Searching")
        Citizen.Wait(100)
        ...
        local finished = exports["drp-taskbar"]:taskBar(5000,"Searching Backseat")
        if luck > 75 and finished then
          if not IsPedInAnyVehicle(PlayerPedId(), false) then
            fuckingRETARDED = false
            return
          end
          local finished = exports["drp-taskbar"]:taskBar(2000,"Found Content")
          DropItemPed()
        end
...
function shutoffenginehotwire()
    ...
    if not fuckingRETARDED then
        ...
        local finished = exports["drp-taskbar"]:taskBar(carTimer,"Attempting Hotwire")
        Citizen.Wait(100)
          ...
          if not hasKey(plate1) then
            ...
            if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
              ...
              if IsEntityDead(pedDriver) then
                ...
                TriggerEvent('drp-dispatch:carjacking')
                local finished = exports["drp-taskbar"]:taskBar(3000,"Taking Keys",false)
                if finished == 100 then
                 -- SetEntityAsMissionEntity(curveh,false,true)
                  ...
                  SetVehicleNeedsToBeHotwired(curveh, false)
                  SetVehicleHasBeenOwnedByPlayer(curveh, true)
                  SetEntityAsMissionEntity(curveh, true, true)
                ...
                if GetEntityModel(curveh) ~= `taxi` then
                  ...
                  if math.random(100) > 95 then
                      ...
                      TriggerEvent('drp-dispatch:carjacking')
                      local finished = exports["drp-taskbar"]:taskBar(3000,"Taking Keys")
                      if finished == 100 then
                       -- SetEntityAsMissionEntity(curveh,false,true)
                        TriggerEvent("keys:addNew",curveh,plate1)
