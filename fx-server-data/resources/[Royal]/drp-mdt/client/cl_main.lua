...
RegisterNetEvent('drp-mdt:RequestOpen')
AddEventHandler('drp-mdt:RequestOpen', function()
	TriggerServerEvent('drp-mdt:OpenMDT')
end)
...
LoadRoyalVersion = function()
    ...
    OpenPublicRecords = function()
        ...
        TriggerEvent('drp-mdt:animation')
        local PlayerInfo = RPC.execute("drp-mdt:getInfo")
    ...
    function EnableGUI(enable)
        if enable then 
            TriggerServerEvent('drp-mdt:opendashboard')
            SendNUIMessage({type = "bulletin", data = RPC.execute("drp-mdt:dashboardbulletin")})
            SendNUIMessage({type = "dispatchmessages", data = RPC.execute("drp-mdt:dashboardMessages")})
            SendNUIMessage({type = "warrants", data = RPC.execute("drp-mdt:getWarrants")})
        end
    ...
    RegisterNetEvent('drp-mdt:getActiveUnits')
    AddEventHandler('drp-mdt:getActiveUnits', function(lspd, bcso, sasp, ems)
        SendNUIMessage({ type = "getActiveUnits", lspd = lspd, bcso = bcso, sasp = sasp, ems = ems })
    end)
    AddEventHandler('drp-mdt:animation', function()
        if not isOpen then 
            return 
        end
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("drp-mdt:deleteBulletin", data.id)
    ...
    RegisterNUICallback("updateLicence", function(data, cb)
        ...
        local cid = data.cid
        RPC.execute("drp-mdt:updateLicence", type, status, cid)
    end)
    RegisterNUICallback("newBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("drp-mdt:newBulletin", data.title, data.info, data.time)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
        if job == "police" or job == "state" or job == "sheriff" and exports['isPed']:isPed("myjob") == "police" or exports['isPed']:isPed("myjob") == "state" or exports['isPed']:isPed("myjob") == "sheriff" then
    ...
    RegisterNetEvent('drp-mdt:open')
    AddEventHandler('drp-mdt:open', function(job, lastname, firstname)
        open = true
        EnableGUI(open)
        local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
        local currentStreetHash, intersectStreetHash = GetStreetNameAtCoord(x, y, z)
    ...
    RegisterNUICallback("searchProfiles", function(data, cb)
        SendNUIMessage({type = "profiles", data = RPC.execute("drp-mdt:searchProfile", data.name), isLimited = RPC.execute("drp-mdt:isLimited")})
    end)
    RegisterNUICallback("saveProfile", function(data, cb)
        RPC.execute("drp-mdt:saveProfile", data.pfp, data.description, data.id)
    end)
    RegisterNUICallback("newWeapon", function(data, cb)
        RPC.execute("drp-mdt:addWeapon", data.id, data.serialnumber)
    end)
    RegisterNUICallback("getProfileData", function(data, cb)
        local isLimited = RPC.execute("drp-mdt:isLimited")
        local ProfileData = RPC.execute("drp-mdt:getProfileData", data.id)
    ...
    RegisterNUICallback("newTag", function(data, cb)
        if data.id ~= "" and data.tag ~= "" then
            RPC.execute("drp-mdt:newTag", data.id, data.tag)
        end
    end)
    RegisterNUICallback("removeProfileTag", function(data, cb)
        RPC.execute("drp-mdt:removeProfileTag", data.cid, data.text)
    end)
    RegisterNUICallback("addGalleryImg", function(data, cb)
        RPC.execute("drp-mdt:addGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("removeGalleryImg", function(data, cb)
        RPC.execute("drp-mdt:removeGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("searchIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("drp-mdt:searchIncidents", data.incident)})
    end)
    RegisterNUICallback("getIncidentData", function(data, cb)
        local sentData, sentConvictions = RPC.execute("drp-mdt:getIncidentData", data.id)
    ...
    RegisterNUICallback("incidentSearchPerson", function(data, cb)
        SendNUIMessage({type = "incidentSearchPerson", data = RPC.execute("drp-mdt:incidentSearchPerson", data.name)})
    end)
    RegisterNUICallback("searchBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("drp-mdt:searchBolos", data.searchVal)})
    end)
    RegisterNUICallback("getAllBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("drp-mdt:getAllBolos")})
    end)
    RegisterNUICallback("getAllIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("drp-mdt:getAllIncidents")})
    end)
    RegisterNUICallback("getBoloData", function(data, cb)
        SendNUIMessage({type = "boloData", data = RPC.execute("drp-mdt:getBoloData", data.id)})
    end)
    RegisterNUICallback("jailPlayer", function(data, cb)
        RPC.execute("drp-mdt:JailPlayer", data.cid, data.time)
    end)
    RegisterNUICallback("newBolo", function(data, cb)
        local sentData = RPC.execute("drp-mdt:newBolo", data.existing, data.id, data.title, data.plate, data.owner, data.individual, data.detail, data.tags, data.gallery, data.officers, data.time)
        SendNUIMessage({type = "boloComplete", data = sentData})
    end)
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("drp-mdt:deleteBolo", data.id)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("drp-mdt:deleteIncident", data.id, data.time)
    end)
    RegisterNUICallback("deleteReport", function(data, cb)
        RPC.execute("drp-mdt:deleteReport", data.id, data.time)
    end)
    RegisterNUICallback("deleteICU", function(data, cb)
        RPC.execute("drp-mdt:deleteICU", data.id)
    end)
    RegisterNUICallback("getAllReports", function(data, cb)
        local asdsadassa = RPC.execute("drp-mdt:getAllReports")
        print(json.encode(asdsadassa))
        SendNUIMessage({type = "reports", data = RPC.execute("drp-mdt:getAllReports")})
    end)
    RegisterNUICallback("getReportData", function(data, cb)
        SendNUIMessage({type = "reportData", data = RPC.execute("drp-mdt:getReportData", data.id)})
    end)
    RegisterNUICallback("searchReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("drp-mdt:searchReports", data.name)})
    end)
    RegisterNUICallback("newReport", function(data, cb)
        local sentData = RPC.execute("drp-mdt:newReport", data.existing, data.id, data.title, data.type, data.detail, data.tags, data.gallery, data.officers, data.civilians, data.time)
        SendNUIMessage({type = "reportComplete", data = sentData})
    end)
    RegisterNUICallback("searchVehicles", function(data, cb)
        local sentData = RPC.execute("drp-mdt:searchVehicles", data.name)
        for i = 1, #sentData do
    ...
    RegisterNUICallback("searchWeapon", function(data, cb)
        local sentData = RPC.execute("drp-mdt:searchWeapon", data.name)
        SendNUIMessage({type = "searchedWeapons", data = sentData})
    end)
    RegisterNUICallback("getWeaponData", function(data, cb)
        local sentData = RPC.execute("drp-mdt:getWeaponData", data.serialnumber)
        SendNUIMessage({type = "getWeaponData", data = sentData})
    end)
    RegisterNUICallback("getVehicleData", function(data, cb)
        local sentData = RPC.execute("drp-mdt:getVehicleData", data.plate)
        if sentData then
    ...
    RegisterNUICallback("saveVehicleInfo", function(data, cb)
        RPC.execute("drp-mdt:saveVehicleInfo", data.plate, data.imageurl, data.notes)
    end)
    RegisterNUICallback("saveWeaponInfo", function(data, cb)
        RPC.execute("drp-mdt:saveWeaponInfo", data.serialnumber, data.imageurl, data.brand, data.type, data.notes)
    end)
    RegisterNUICallback("knownInformation", function(data, cb)
        RPC.execute("drp-mdt:knownInformation", data.type, data.status, data.plate)
    end)
    RegisterNUICallback("getAllLogs", function(data, cb)
        SendNUIMessage({type = "getAllLogs", data = RPC.execute("drp-mdt:getAllLogs")})
    end)
    RegisterNUICallback("getPenalCode", function(data, cb)
        local titles, penalcode = RPC.execute("drp-mdt:getPenalCode")
        SendNUIMessage({type = "getPenalCode", titles = titles, penalcode = penalcode})
    end)
    RegisterNUICallback("missingCitizen", function(data, cb)
        RPC.execute("drp-mdt:missingCitizen", data.cid, data.bool)
    end)
    RegisterNUICallback("getAllMissing", function(data, cb)
        SendNUIMessage({type = "missing", data = RPC.execute("drp-mdt:getAllMissing")})
    end)
    RegisterNUICallback("deleteMissing", function(data, cb)
        RPC.execute("drp-mdt:deleteMissing", data.id, data.time)
    end)
    RegisterNUICallback("searchMissing", function(data, cb)
        local sentData = RPC.execute("drp-mdt:searchMissing", data.name)
        SendNUIMessage({type = "searchedMissing", data = sentData})
    end)
    RegisterNUICallback("getMissingData", function(data, cb)
        local sentData = RPC.execute("drp-mdt:getMissingData", data.id)
        SendNUIMessage({type = "getMissingData", data = sentData})
    end)
    RegisterNUICallback("saveMissingInfo", function(data, cb)
        RPC.execute("drp-mdt:saveMissingInfo", data.id, data.imageurl, data.notes)
    end)
    RegisterNUICallback("toggleDuty", function(data, cb)
        RPC.execute("drp-mdt:toggleDuty", data.cid, data.status)
    end)
    ...
    RegisterNUICallback("saveIncident", function(data, cb)
        ...
        TriggerClientEvent('DoLongHudText', string, steamIdentifier, 1)
    ...
    RegisterNUICallback("removeIncidentCriminal", function(data, cb)
        RPC.execute("drp-mdt:removeIncidentCriminal", data.cid, data.incidentId)
    end)
    RegisterNUICallback("setWaypoint", function(data, cb)
        local callInformation = RPC.execute("drp-mdt:setWaypoint", data.callid)
    ...
    RegisterNUICallback("callDetach", function(data, cb)
        local callid, sentData = RPC.execute("drp-mdt:callDetach", data.callid)
    ...
    RegisterNUICallback("removeCall", function(data, cb)
        local src, callid = RPC.execute("drp-mdt:removeCall", data.callid)
    ...
    RegisterNUICallback("callAttach", function(data, cb)
        local callid, sentData = RPC.execute("drp-mdt:callAttach", data.callid)
        local job = exports['isPed']:isPed("myjob")
        if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then
    ...
    RegisterNUICallback("attachedUnits", function(data, cb)
        local sentData, callid = RPC.execute("drp-mdt:attachedUnits", data.callid)
        SendNUIMessage({type = "attachedUnits", data = sentData, callid = callid})
    end)
    RegisterNUICallback("callDispatchDetach", function(data, cb)
        RPC.execute("drp-mdt:callDispatchDetach", data.callid, data.cid)
    end)
    RegisterNUICallback("setDispatchWaypoint", function(data, cb)
        local callInformation = RPC.execute("drp-mdt:setDispatchWaypoint", data.callid)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDragAttach", function(data, cb)
        RPC.execute("drp-mdt:callDragAttach", data.callid, data.cid)
    end)
    RegisterNUICallback("setWaypointU", function(data, cb)
        local sentData = RPC.execute("drp-mdt:setWaypoint:unit", data.cid)
        SetNewWaypoint(sentData.x, sentData.y) 
    end)
    RegisterNUICallback("dispatchMessage", function(data, cb)
        local sentData = RPC.execute("drp-mdt:sendMessage", data.message, data.time)
        local job = exports['isPed']:isPed("myjob")
        if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then
    ...
    RegisterNUICallback("refreshDispatchMsgs", function(data, cb)
        SendNUIMessage({type = "dispatchmessages", data = RPC.execute("drp-mdt:refreshDispatchMsgs")})
    end)
end
