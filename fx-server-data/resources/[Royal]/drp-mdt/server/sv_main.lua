...
RegisterServerEvent('drp-dispatch:GrabRadioChannel')
AddEventHandler("drp-dispatch:GrabRadioChannel", function(radio)
	local src = source
...
RegisterServerEvent('drp-mdt:OpenMDT')
AddEventHandler("drp-mdt:OpenMDT", function()
	local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	if userjob == "police" or userjob == "sheriff" or userjob == "state" or userjob == "ems" or userjob == "judge" or userjob == "district_attorney" or userjob == "public_defender" then
        TriggerClientEvent('drp-mdt:open', src, userjob, char.last_name, char.first_name)
        TriggerClientEvent('drp-voice:getRadioChannel', src)
	end
end)

RegisterServerEvent("drp-mdt:opendashboard")
AddEventHandler("drp-mdt:opendashboard", function()
    local src = source
	UpdateUnits(src)	
end) 
...
LoadRoyalVersion = function()
    GetUserByCid = function(cid)
        for index, value in pairs(GetPlayers()) do
            local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(tonumber(value))
    ...
    GetCharData = function(source)
        local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(source)
        local xChar = xPlayer:getCurrentCharacter()
    ...
    CheckType = function(src)
        local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("drp-mdt:getInfo", function()
        return {firstname = GetCharData(source).first_name, lastname = GetCharData(source).last_name}
    end)
    RPC.register("drp-mdt:dashboardbulletin", function()
        return GetAllBulletinData()
    end)
    RPC.register("drp-mdt:dashboardMessages", function()
        return GetAllMessages()
    end)
    RPC.register("drp-mdt:getWarrants", function()
        local WarrantData = {}
        local data = SQL("SELECT * FROM mdw_incidents2", {})
    ...
    function UpdateUnits(src)	
        ...
        for k, v in pairs(GetPlayers()) do
        local user = exports["drp-base"]:getModule("Player"):GetUser(tonumber(v))
            if user then
                local userjob = user:getVar("job")
        ...
        TriggerClientEvent("drp-mdt:getActiveUnits", src, lspd, bcso, sasp, ems)
    end	    
    ...
    RPC.register("drp-mdt:getProfileData", function(identifier)
        local ReturnData = {}
        local result = SQL('SELECT * FROM characters WHERE id = @id', {['@id'] = identifier})
    ...
    RPC.register("drp-mdt:JailPlayer", function(cid, time)
        JailPlayer(source, cid, time)
    end)
    RPC.register("drp-mdt:saveProfile", function(profilepic, information, cid)
    ...
    RPC.register("drp-mdt:addGalleryImg", function(cid, url)
    ...
    RPC.register("drp-mdt:addWeapon", function(cid, serialnumber)
    ...
    RPC.register("drp-mdt:removeGalleryImg", function(cid, url)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = cid
        }, function(result)
            local NewGallery = {}
    ...
    RPC.register("drp-mdt:newTag", function(identifier, tag)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = identifier
        }, function(result)
            local NewTags = {}
    ...
    RPC.register("drp-mdt:removeProfileTag", function(cid, tagtext)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = cid
        }, function(result)
            local NewTags = {}
    ...
    RPC.register("drp-mdt:updateLicence", function(type, status, cid)
        local src = source
        local user = exports["drp-base"]:getModule("Player"):GetUser(src)	
        local char = user:getCurrentCharacter()

        if status == "revoke" then
            exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['status'] = '0', ['cid'] = cid, ['type'] = type})
        elseif status == 'give' then
            exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['status'] = '1', ['cid'] = cid, ['type'] = type})
        end
    end)
    RPC.register("drp-mdt:deleteBulletin", function(id, time)
        local src = source
        local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("drp-mdt:newBulletin", function(title, info, time)
        local src = source
        local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("drp-mdt:searchProfile", function(name)
        local Matches = {}
    ...
    RPC.register("drp-mdt:searchIncidents", function(incident)
        local Matches = {}
        local incidents = SQL('SELECT * FROM mdw_incidents2', {})
    ...
    RPC.register("drp-mdt:saveIncident", function(ID, title, information, tags, officers, civilians, evidence, associated, time)
        ...
        local result = SQL('SELECT * FROM mdw_incidents2 WHERE id = @id', {['@id'] = ID})
        TriggerClientEvent('DoLongHudText', src, result, 1)
        if result[1] then
            SQL("UPDATE mdw_incidents2 SET title = @title, information = @information, tags = @tags, officers = @officers, civilians = @civilians, evidence = @evidence, associated = @associated, time = @time, author = @author WHERE id = @id", {
                ['@id'] = ID,
                ['@title'] = title, 
            ...
            exports.ghmattimysql:execute('INSERT INTO mdw_incidents2 (title, information, tags, officers, civilians, evidence, associated, time, author) VALUES (@title, @information, @tags, @officers, @civilians, @evidence, @associated, @time, @author)', {
                ['@id'] = ID,
                ['@title'] = title, 
                ['@information'] = information, 
                ['@tags'] = json.encode(tags),
            ...
            TriggerClientEvent('DoLongHudText', "After mysql request", string, 1)
    ...
    RPC.register("drp-mdt:getAllIncidents", function()
        local Tables = {}
        local results = SQL('SELECT * FROM mdw_incidents2', {})
        for index, data in pairs(results) do
    ...
    RPC.register("drp-mdt:incidentSearchPerson", function(name)
        local Matches = {}
        local users = SQL("SELECT * FROM characters", {})
        for index = 1, #users, 1 do
            local firstname = string.lower(users[index].first_name)
    ...
    RPC.register("drp-mdt:removeIncidentCriminal", function(cid, incidentId)
        local result = SQL('SELECT * FROM mdw_incidents2 WHERE id = @id', {['@id'] = incidentId})
        if result[1] then
            local Table = {}
            ...
            SQL("UPDATE mdw_incidents2 SET associated = @associated WHERE id = @id", {
                ['@associated'] = json.encode(Table),
                ['@id'] = incidentId
            })
        end
    end)
    RPC.register("drp-mdt:getPenalCode", function()
        return Config['OffensesTitels'], Config["Offenses"]
    end)
    RPC.register("drp-mdt:missingCitizen", function(cid, time)
        local result = SQL('SELECT * FROM characters WHERE id = @id', {['@id'] = cid})
    ...
    RPC.register("drp-mdt:getAllMissing", function()
        local Tables = {}
    ...
    RPC.register("drp-mdt:deleteMissing", function(id, time)
        local src = source
    ...
    RPC.register("drp-mdt:saveMissingInfo", function(id, imageurl, notes)
    ...
    RPC.register("drp-mdt:searchMissing", function(name)
        local ReturnData = {}
    ...
    RPC.register("drp-mdt:getMissingData", function(id)
    ...
    RPC.register("drp-mdt:searchBolos", function(searchVal)
        local Matches = {}
        local BolosData = SQL("SELECT * FROM mdt_bolos", {})
        for index = 1, #BolosData, 1 do
    ...
    RPC.register("drp-mdt:getAllBolos", function()
        local Tables = {}
        local BolosData = SQL("SELECT * FROM mdt_bolos", {})
        for index = 1, #BolosData, 1 do
    ...
    RPC.register("drp-mdt:getBoloData", function(id)
        local result = SQL('SELECT * FROM mdt_bolos WHERE id = @id', {['@id'] = id})
        return result[1]
    end)
    RPC.register("drp-mdt:newBolo", function(existing, id, title, plate, owner, individual, detail, tags, gallery, officers, time)
        local src = source
        local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("drp-mdt:deleteBolo", function(id, time)
        local src = source
    ...
    RPC.register("drp-mdt:deleteReport", function(id, time)
        local src = source
    ...
    RPC.register("drp-mdt:deleteIncident", function(id, time)
        local src = source
    ...
    RPC.register("drp-mdt:deleteICU", function(id)
        SQL("DELETE FROM mdt_bolos WHERE id = @id", {['@id'] = id})
    end)
    RPC.register("drp-mdt:getAllReports", function()
        local Tables = {}
    ...
    RPC.register("drp-mdt:getReportData", function(id)
        local result = SQL('SELECT * FROM mdt_report WHERE id = @id', {['@id'] = id})
        return result[1]
    end)
    RPC.register("drp-mdt:searchReports", function(name)
        local Matches = {}
    ...
    RPC.register("drp-mdt:newReport", function(existing, id, title, reporttype, detail, tags, gallery, officers, civilians, time)
        local src = source
        if not existing then
    ...
    RPC.register("drp-mdt:searchVehicles", function(name)
        local ReturnData = {}
    ...
    RPC.register("drp-mdt:searchWeapon", function(name)
        local ReturnData = {}
    ...
    RPC.register("drp-mdt:getWeaponData", function(serialnumber)
        local result = SQL('SELECT * FROM weapons_serials WHERE serial = @serial', {['@serial'] = serialnumber})
        if result[1] then
            return result[1]
        end
    end)
    RPC.register("drp-mdt:saveWeaponInfo", function(serialnumber, imageurl, brand, type, notes)
    ...
    RPC.register("drp-mdt:getVehicleData", function(plate)
    ...
    RPC.register("drp-mdt:saveVehicleInfo", function(plate, imageurl, notes)
        local result = SQL('SELECT * FROM mdt_vehicleinfo WHERE plate = @plate', {['@plate'] = plate})
        if result[1] then
    ...
    RPC.register("drp-mdt:knownInformation", function(type, status, plate)
        local result = SQL('SELECT * FROM mdt_vehicleinfo WHERE plate = @plate', {['@plate'] = plate})
        if result[1] then
            if type == "code5" then
    ...
    RPC.register("drp-mdt:getAllLogs", function()
        local MDTLogs = {}
        return MDTLogs
    end)
    RPC.register("drp-mdt:toggleDuty", function(cid, status)
        local src = source
        ChangePlayerDuty(src, status)
    end)
    RegisterServerEvent("drp-mdt:setCallsign", function(cid, callsign)
    ...
    RPC.register("drp-mdt:callDetach", function(callid)
        local src = source
        local NewTable = {}
    ...
    RPC.register("drp-mdt:removeCall", function(callid)
        MDTDispatchData[callid] = nil
        return source, callid
    end)
    RPC.register("drp-mdt:callAttach", function(callid)
        local src = source
    ...
    RPC.register("drp-mdt:attachedUnits", function(callid)
        local ReturnData = {}
        for index, data in pairs(AttachedUnits) do
            if data.CallId == callid then
                local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(data.Source)
                local xChar = xPlayer:getCurrentCharacter()
                table.insert(ReturnData, {
                    cid = xChar.id,
                    job = xPlayer.job,
                    callsign = pCallSign(tonumber(xChar.id)) or "No Callsign",
    ...
    RPC.register("drp-mdt:callDispatchDetach", function(callid, cid)
    ...
    RPC.register("drp-mdt:setDispatchWaypoint", function(callid)
        return MDTDispatchData[callid]
    end)
    RPC.register("drp-mdt:callDragAttach", function(callid, cid)
        local xPlayer = GetUserByCid(tostring(cid))
        for _, data in pairs(AttachedUnits) do
            if data.CallId == callid then
    ...
    RPC.register("drp-mdt:sendMessage", function(message, time)
        local src = source
        local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(src)
        local xChar = xPlayer:getCurrentCharacter()
        local ReturnData = {}
    ...
    RPC.register("drp-mdt:refreshDispatchMsgs", function()
        return GetAllMessages()
    end)
    RPC.register("drp-mdt:setWaypoint:unit", function(cid)
        local xPlayer = GetUserByCid(cid)
        local xPlayerCoords = GetEntityCoords(GetPlayerPed(xPlayer.source))
        return xPlayerCoords
    end)
    RPC.register("drp-mdt:isLimited", function()
        local xPlayer = exports["drp-base"]:getModule("Player"):GetUser(source)
