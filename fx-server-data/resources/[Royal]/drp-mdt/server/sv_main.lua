...
LoadRoyalVersion = function()
    ...
    RPC.register("drp-mdt:getWarrants", function()
        local WarrantData = {}
        local data = SQL("SELECT * FROM mdt_incidents2", {})
    ...
    RPC.register("drp-mdt:searchIncidents", function(incident)
        local Matches = {}
        local incidents = SQL('SELECT * FROM mdt_incidents2', {})
    ...
    RPC.register("drp-mdt:saveIncident", function(ID, title, information, tags, officers, civilians, evidence, associated, time)
        ...
        local result = SQL('SELECT * FROM mdt_incidents2 WHERE id = @id', {['@id'] = ID})
        TriggerClientEvent('DoLongHudText', src, result, 1)
        if result[1] then
            SQL("UPDATE mdt_incidents2 SET title = @title, information = @information, tags = @tags, officers = @officers, civilians = @civilians, evidence = @evidence, associated = @associated, time = @time, author = @author WHERE id = @id", {
                ['@id'] = ID,
                ['@title'] = title, 
            ...
            exports.ghmattimysql:execute('INSERT INTO mdt_incidents2 (title, information, tags, officers, civilians, evidence, associated, time, author) VALUES (@title, @information, @tags, @officers, @civilians, @evidence, @associated, @time, @author)', {
                ['@id'] = ID,
                ['@title'] = title, 
                ['@information'] = information, 
                ['@tags'] = json.encode(tags),
    ...
    RPC.register("drp-mdt:getAllIncidents", function()
        local Tables = {}
        local results = SQL('SELECT * FROM mdt_incidents2', {})
        for index, data in pairs(results) do
    ...
    RPC.register("drp-mdt:removeIncidentCriminal", function(cid, incidentId)
        local result = SQL('SELECT * FROM mdt_incidents2 WHERE id = @id', {['@id'] = incidentId})
        if result[1] then
            local Table = {}
            ...
            SQL("UPDATE mdt_incidents2 SET associated = @associated WHERE id = @id", {
