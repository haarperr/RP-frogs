RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("drp-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("drp-musicplayer:stateSound")
AddEventHandler("drp-musicplayer:stateSound", function(state, data)
