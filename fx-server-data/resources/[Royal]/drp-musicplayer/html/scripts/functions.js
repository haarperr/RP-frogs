    ...
    if(howler){
        for (var soundName in soundList)
        {
            ...
            if(sound.loaded() == false){
                ...
                $.post('https://drp-musicplayer/events', JSON.stringify(
                {
                    type: "onPlay",
                    id: sound.getName(),
                }));
                ...
                $.post('https://drp-musicplayer/data_status', JSON.stringify(
                {
                    time: time,
                    type: "maxDuration",
                    id: sound.getName(),
...
function isLooped(divId){
	for (var soundName in soundList)
	{
        ...
        if(sound.getDivId() === divId && sound.isLoop()){
            ...
            $.post('https://drp-musicplayer/data_status', JSON.stringify({ type: "finished",id: soundName }));
            $.post('https://drp-musicplayer/events', JSON.stringify(
            {
                type: "onEnd",
                id: sound.getName(),
