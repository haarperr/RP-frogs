function PlayUrl(source, name_, url_, volume_, loop_)
    TriggerClientEvent("drp-musicplayer:stateSound", source, "play", {
        soundId = name_,
        url = url_,
        volume = volume_,
        loop = loop_
...
function PlayUrlPos(source, name_, url_, volume_, pos, loop_)
    TriggerClientEvent("drp-musicplayer:stateSound", source, "playpos", {
        soundId = name_,
        position = pos,
        url = url_,
        volume = volume_,
...
function TextToSpeech(source, name_, lang, text, volume_, loop_)
    TriggerClientEvent("drp-musicplayer:stateSound", source, "texttospeech", {
        soundId = name_,
        url = text,
        lang = lang,
        volume = volume_,
...
function TextToSpeechPos(source, name_, lang, text, volume_, pos, loop_)
    TriggerClientEvent("drp-musicplayer:stateSound", source, "texttospeechpos", {
        soundId = name_,
        lang = lang,
        position = pos,
        url = text,
...
RegisterServerEvent("cluckinbell:request:song:sv", function(url)
    local Speaker3 = vector3(-517.5198, -692.0820, 34.6995)
    TriggerClientEvent("play:song", -1, url, Speaker3, "BU")
    local data = {
        soundId = "CB",
        distance =  40,
