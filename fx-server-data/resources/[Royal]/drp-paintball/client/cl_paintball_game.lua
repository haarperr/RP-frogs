...
local function drawStatusHud()
  ...
  if gameInProgress or (teamPlayersEast + teamPlayersWest > 0) then
    ...
    exports["drp-ui"]:sendAppEvent("status-hud", { show = false })
  end
end

AddEventHandler("drp-paintball:game:interact", function(pArgs)
  ...
  RPC.execute("drp-paintball:game:action", action, ctx)
end)

RegisterNetEvent("drp-paintball:game:update", function(pCtx)
  ...
  --     dui = nil,
  --   }
  --   conf.dui = exports["drp-lib"]:getDui(url, 512, 512)
  --   print(conf.dui)
...
AddEventHandler("drp-paintball:enterStadium", function()
  inStadium = true
  drawStatusHud()
end)
AddEventHandler("drp-paintball:leaveStadium", function()
  inStadium = false
  exports["np-ui"]:sendAppEvent("status-hud", { show = false })
end)

Citizen.CreateThread(function()
  exports["drp-interact"]:AddPeekEntryByModel({ flagPropHash }, {
    {
      id = "paintballflag",
      event = "drp-paintball:game:pickupFlag",
      icon = "circle",
      label = "Pickup",
    },
...
RegisterNetEvent("drp-paintball:game:spawnFlag", function(pCoords)
  local flagObj = CreateObject(flagPropHash, pCoords, 1, 1, 0)
  Wait(0)
  TriggerServerEvent("drp-paintball:game:spawnedFlag", NetworkGetNetworkIdFromEntity(flagObj))
end)
...
AddEventHandler("drp-paintball:game:pickupFlag", function()
  ...
  isCarryingFlag = true
  TriggerEvent("drp-props:attachProp", flagProp, 24818, -0.25, -0.25, 0.0, 180.0, 90.0, 0.0)
  TriggerServerEvent("drp-paintball:game:pickedUpFlag")
  -- Wait(5000)
  -- removeAttachedProp()
end)

local isDead = false
AddEventHandler("drp:deathcheck", function()
  isDead = not isDead
  ...
  TriggerServerEvent("drp-paintball:game:diedWithFlag", coords)
end)
