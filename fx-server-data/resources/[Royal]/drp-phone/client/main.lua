...
RegisterNetEvent('blood:email:suc')
AddEventHandler('blood:email:suc',function(value)
	TriggerEvent("phone:addnotification","Gov", "Your request for a blood search on our database has come back with the name: ".. value .." ~ <b>Regards National DNA Database of America</b>")
end)

RegisterNetEvent('blood:email:fail')
AddEventHandler('blood:email:fail',function()
	TriggerEvent("phone:addnotification","Gov", "Your request for a blood search on our database has come back: NO MATCH ~ <b>Regards National DNA Database of America</b>")
end)

RegisterNetEvent('blood:email:request')
AddEventHandler('blood:email:request',function()
	TriggerEvent("phone:addnotification","Gov", "Your request has been logged and added to the queue, Please standby... <b>Regards National DNA Database of America</b>")
end)

RegisterNetEvent('serial:email:suc')
AddEventHandler('serial:email:suc',function(value)
	TriggerEvent("phone:addnotification","Gov", "Your request for a serial search on our database has come back with the name: ".. value .." ~ <b>Regards Department of Firearms</b>")
end)

RegisterNetEvent('serial:email:fail')
AddEventHandler('serial:email:fail',function()
	TriggerEvent("phone:addnotification","Gov", "Your request for a serial search on our database has come back: NO MATCH ~ <b>Regards Department of Firearms</b>")
end)

RegisterNetEvent('serial:email:request')
AddEventHandler('serial:email:request',function()
	TriggerEvent("phone:addnotification","Gov", "Your request has been logged and added to the queue, Please standby... ~ <b>Regards Department of Firearms</b>")
end)
...
function initiatingCall()
  ...
  while (callTimer > 0 and callStatus == isDialing) do
    TriggerEvent('InteractSound_CL:PlayOnOne', 'phoneoutgoing', 0.4)
    Citizen.Wait(2500)
...
AddEventHandler('phone:transaction', function(name, message)
    SendNUIMessage({openSection = "transaction", pEHandle = 'Transaction', pEMessages = message})
    if exports['drp-phone']:pOpen() == false then 
      SendNUIMessage({openSection = "transaction", pEHandle = 'Transaction', pEMessages = message})
      SendNUIMessage({openSection = "phonemedio", timeout = "5200", pOpen = exports['drp-phone']:pOpen()}) 
    end
...
AddEventHandler('phone:triggerPager', function()
  ...
  if job == "doctor" or job == "ems" then
    ...
    if lastTime == 0 or lastTime + (5 * 60 * 1000) < currentTime then
      TriggerEvent('InteractSound_CL:PlayOnOne', 'phone', 1.0)
      SendNUIMessage({
...
RegisterNUICallback('SetFishingNewWaypoint', function()
  ...
  SetNewWaypoint(1530.6593017578,3776.8879394531)
end)
