    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('drp-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('drp-ping:client:SendPing')
AddEventHandler('drp-ping:client:SendPing', function(sender, senderId, pCoords)
    if pendingPing == nil then
        ...
        TriggerServerEvent('drp-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('drp-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('drp-ping:client:AcceptPing')
AddEventHandler('drp-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('drp-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('drp-ping:client:RejectPing')
AddEventHandler('drp-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('drp-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('drp-ping:client:RemovePing')
AddEventHandler('drp-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
...
RegisterCommand('ping', function(src, args, raw)
    TriggerServerEvent("drp-ping:attempt",  GetEntityCoords(PlayerPedId()), args[1])
end)
