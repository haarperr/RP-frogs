...
AddEventHandler('police:targetCheckBank', function(target)
	local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('checkLicensePlate', function(oof)
	...
	local kekw = oof
	exports.ghmattimysql:execute('SELECT cid FROM characters_cars WHERE `license_plate` = @plate', { ['@plate'] = kekw }, function(result)
		if result[1] ~= nil then
			exports.ghmattimysql:execute('SELECT * FROM characters WHERE `id` = @cid', { ['@cid'] = result[1].cid }, function(data)
				exports.ghmattimysql:execute('SELECT * FROM jobs_whitelist WHERE `cid` = @cid', { ['@cid'] = data[1].id }, function(penis)
...
AddEventHandler("serial:search:weapon", function(serial)
	local src = source
	if serial ~= nil then
		exports.ghmattimysql:execute('SELECT * FROM weapons_serials WHERE `serial`= ?', {serial}, function(data)
...
RegisterServerEvent("blood:search:weapon")
AddEventHandler("blood:search:weapon", function(blood)
	local src = source
	if blood ~= nil then
		exports.ghmattimysql:execute('SELECT * FROM characters WHERE `id`= ?', {blood}, function(data)
			TriggerClientEvent("DoLongHudText", src, "Keep an eye on your email")
			Wait(5000)
			...
			TriggerClientEvent('blood:email:suc', src, responceFromDB)
				...
				TriggerClientEvent('blood:email:fail', src)
			end
		end)
	end
end)
...
AddEventHandler("police:targetCheckInventory", function(target, status)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:jailGranted', function(args)
	...
	local target = exports["drp-base"]:getModule("Player"):GetUser(player)
	local character = target:getCurrentCharacter()
	local playerName = character.first_name .. ' ' .. character.last_name

	local pdunit = exports["drp-base"]:getModule("Player"):GetUser(src)
	...
	exports["drp-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
end)
...
AddEventHandler('police:dnaAsk', function(t)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(t)
...
AddEventHandler("911", function(args)
	...
	local users = exports["drp-base"]:getModule("Player"):GetUsers()

	local emergencyPlayers = {}

	for k,v in pairs(users) do
		local user = exports["drp-base"]:getModule("Player"):GetUser(v)
		local job = user:getVar("job")
...
RegisterServerEvent('drp-base:Licenses')
AddEventHandler('drp-base:Licenses', function()
    local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
    exports.ghmattimysql:execute("SELECT * FROM user_licenses WHERE cid = ? AND status = ? AND type = ?", {char.id, "1", "Weapon License"}, function(result)
        if (result[1]) then
...
RegisterServerEvent('drp-base:updatedphoneLicenses')
AddEventHandler('drp-base:updatedphoneLicenses', function()
	local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	local licenceTable = {}
	exports.ghmattimysql:execute("SELECT type, status FROM user_licenses WHERE cid = @cid",{['cid'] = char.id}, function(result)
		for i=1, #result do
...
RegisterServerEvent('drp-base:addLicenses')
AddEventHandler('drp-base:addLicenses', function()
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(src)
	local char2 = user2:getCurrentCharacter()
	exports.ghmattimysql:execute("SELECT * FROM user_licenses WHERE cid = ?", {char2.id}, function(result)
		if result[1] == nil then
			exports.ghmattimysql:execute("INSERT INTO user_licenses (cid, type, status) VALUES (@cid, @type, @status)", {['@cid'] = char2.id, ['@type'] = 'Weapon', ['@status'] = '0'})
			exports.ghmattimysql:execute("INSERT INTO user_licenses (cid, type, status) VALUES (@cid, @type, @status)", {['@cid'] = char2.id, ['@type'] = 'Drivers', ['@status'] = '1'})
			exports.ghmattimysql:execute("INSERT INTO user_licenses (cid, type, status) VALUES (@cid, @type, @status)", {['@cid'] = char2.id, ['@type'] = 'Hunting', ['@status'] = '0'})
			exports.ghmattimysql:execute("INSERT INTO user_licenses (cid, type, status) VALUES (@cid, @type, @status)", {['@cid'] = char2.id, ['@type'] = 'Fishing', ['@status'] = '0'})
			exports.ghmattimysql:execute("INSERT INTO user_licenses (cid, type, status) VALUES (@cid, @type, @status)", {['@cid'] = char2.id, ['@type'] = 'Bar', ['@status'] = '0'})
			exports.ghmattimysql:execute("INSERT INTO user_licenses (cid, type, status) VALUES (@cid, @type, @status)", {['@cid'] = char2.id, ['@type'] = 'Business', ['@status'] = '0'})
			exports.ghmattimysql:execute("INSERT INTO user_licenses (cid, type, status) VALUES (@cid, @type, @status)", {['@cid'] = char2.id, ['@type'] = 'Pilot', ['@status'] = '0'})
		end
	end)
end)
...
RegisterServerEvent('drp-base:giveLicenses')
AddEventHandler('drp-base:giveLicenses', function(t)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
	exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['@cid'] = char2.id, ['@type'] = 'Weapon License', ['@status'] = '1'})
	...
	Citizen.Wait(2000)
	local licenceTable = {}
	exports.ghmattimysql:execute("SELECT type, status FROM user_licenses WHERE cid = @cid",{['cid'] = char2.id}, function(result)
		for i=1, #result do
			table.insert(licenceTable,{
				type = result[i].type,
				status = result[i].status
...
RegisterServerEvent('drp-base:removeLicenses')
AddEventHandler('drp-base:removeLicenses', function(t)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
	exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['@cid'] = char2.id, ['@type'] = 'Weapon License', ['@status'] = '0'})
...
RegisterServerEvent('drp-base:giveLicenseshunt')
AddEventHandler('drp-base:giveLicenseshunt', function(t)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
	exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['@cid'] = char2.id, ['@type'] = 'Hunting License', ['@status'] = '1'})
...
RegisterServerEvent('drp-base:removeLicenseshunt')
AddEventHandler('drp-base:removeLicenseshunt', function(t)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
	exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['@cid'] = char2.id, ['@type'] = 'Hunting License', ['@status'] = '0'})
...
RegisterServerEvent('drp-base:giveLicensesfish')
AddEventHandler('drp-base:giveLicensesfish', function(t)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
	exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['@cid'] = char2.id, ['@type'] = 'Fishing License', ['@status'] = '1'})
	user2:removeMoney(200)
...
RegisterServerEvent('drp-base:removeLicensesfish')
AddEventHandler('drp-base:removeLicensesfish', function(t)
	local src = source
	local user2 = exports["drp-base"]:getModule("Player"):GetUser(t)
	local char2 = user2:getCurrentCharacter()
	exports.ghmattimysql:execute("UPDATE user_licenses SET `status` = @status WHERE cid = @cid AND type = @type", {['@cid'] = char2.id, ['@type'] = 'Fishing License', ['@status'] = '0'})
...
AddEventHandler('police:getLicenses', function(t)
	local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(t)
	local char = user:getCurrentCharacter()
	exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = ? AND type = ?", {char.id, "Weapon"}, function(result)
		exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = ? AND type = ?", {char.id, "Drivers"}, function(result2)
			exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = ? AND type = ?", {char.id, "Hunting"}, function(result3)
				exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = ? AND type = ?", {char.id, "Fishing"}, function(result4)
					exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = ? AND type = ?", {char.id, "Bar"}, function(result5)
						exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = ? AND type = ?", {char.id, "Business"}, function(result6)
							exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = ? AND type = ?", {char.id, "Pilot"}, function(result7)
...
AddEventHandler("weapons:set:data", function(source, weapon)
	...
	local user = exports["drp-base"]:getModule("Player"):GetUser(src)
	local character = user:getCurrentCharacter()
    exports.ghmattimysql:execute('SELECT * FROM characters WHERE `id`= ?', {character.id}, function(name)
        exports.ghmattimysql:execute('INSERT INTO weapons_serials (cid, serial, first_name, last_name) VALUES(?, ?, ?, ?)', {character.id, weapon, name[1].first_name, name[1].last_name})
    end)    
end)
...
AddEventHandler("weapons:get:data", function(weapon)
    local src = source
    exports.ghmattimysql:execute('SELECT * FROM weapons_serials WHERE `serial`= ?', {weapon}, function(data)
...
RegisterCommand('bill', function(source, args)
	...
	exports.ghmattimysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'police' or result[1].job == 'sheriff'  or result[1].job == 'state' or result[1].job == 'judge' then
			...
			TriggerClientEvent("DoLongHudText", src, "your a clown",2)
		end
	end)
end)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "heat" and v.rank >= 5 then
				TriggerClientEvent('drp-jobsystem:heatJobMenuHire', src)
			end
		end
	end)
end)
...
RegisterCommand("fireHeat", function(source)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "heat" and v.rank >= 5 then
				TriggerClientEvent('drp-jobsystem:heatJobMenuFire', src)
			end
		end
	end)
end)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "casino" and v.rank >= 4 then
				TriggerClientEvent('drp-jobsystem:casinoJobMenuHire', src)
			end
		end
	end)
end)
...
RegisterCommand("firecasino", function(source)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "casino" and v.rank >= 4 then
				TriggerClientEvent('drp-jobsystem:casinoJobMenuFire', src)
			end
		end
	end)
end)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "yakuza" and v.rank >= 5 then
				TriggerClientEvent('drp-jobsystem:yakuzaJobMenuHire', src)
			end
		end
	end)
end)
...
RegisterCommand("fireyakuza", function(source)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "yakuza" and v.rank >= 5 then
				TriggerClientEvent('drp-jobsystem:yakuzaJobMenuFire', src)
			end
		end
	end)
end)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "bondi_boys" and v.rank >= 4 then
				TriggerClientEvent('drp-jobsystem:bondiJobMenuHire', src)
			end
		end
	end)
end)
...
RegisterCommand("firebondi", function(source)
	...
	exports.ghmattimysql:execute('SELECT * FROM character_passes WHERE cid = ?', {cid}, function(result)
		for i, v in pairs(result) do
			if v.name == "bondi_boys" and v.rank >= 4 then
				TriggerClientEvent('drp-jobsystem:bondiJobMenuFire', src)
			end
		end
	end)
end)