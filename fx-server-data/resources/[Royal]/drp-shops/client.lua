		...
		Citizen.Wait(1)
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
				...
				if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
					...
					local finished = exports["drp-taskbar"]:taskBar(60000,"Searching...")
...
RegisterNetEvent('drp-shops:exchangeGold')
AddEventHandler('drp-shops:exchangeGold', function()
	if exports["drp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		...
		if finished == 100 then
			if exports["drp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
				TriggerServerEvent('drp-banking:addMoney', 35000)
...
AddEventHandler('evidence:general', function()
	...
	if (job == "police" or job == "sheriff" or job == "state") then
		...
		TriggerEvent('InteractSound_CL:PlayOnOne', 'LockerOpen', 1.0)
		Wait(1000)
	else
	end
end)
...
AddEventHandler('slushy:general', function()
	local finished = exports["drp-taskbar"]:taskBar(60000,"Making a god slushy...")
	if (finished == 100) then
...
AddEventHandler('lockpickshit:general', function()
	local finished = exports["drp-taskbar"]:taskBar(60000,"What dis????")
	if (finished == 100) then
...
AddEventHandler('smelt:crafting', function()
	local finished = exports['drp-taskbar']:taskBar(20000, 'Smelting')
	if (finished == 100) then
		TriggerEvent("server-inventory-open", "6969", "Craft");	
		Wait(1000)
	end
end)
...
RegisterNetEvent("drp-shops:redCounter")
AddEventHandler("drp-shops:redCounter", function()
    TriggerEvent("server-inventory-open", "1", "redcircle_counter");
end)

RegisterNetEvent("drp-shops:redCounter2")
AddEventHandler("drp-shops:redCounter2", function()
...
AddEventHandler('weapon:general', function()
	TriggerServerEvent('drp-shops:WeaponShopStatus')
end)
...
AddEventHandler("hunting:level1", function()
	if exports["drp-inventory"]:getQuantity("coyotepelt") >= 1 then
		playerAnim()
		local finished = exports["drp-taskbar"]:taskBar(4000,"Selling Coyotepelt",true,false,playerVeh)
		if finished == 100 then
			if exports["drp-inventory"]:getQuantity("coyotepelt") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'coyotepelt', 1)
				TriggerServerEvent('drp-banking:addMoney', math.random(80, 150))
			else
...
AddEventHandler("hunting:level2", function()
	if exports["drp-inventory"]:getQuantity("buck") >= 1 then
		playerAnim()
		local finished = exports["drp-taskbar"]:taskBar(4000,"Selling Buck",true,false,playerVeh)
		if finished == 100 then
			if exports["drp-inventory"]:getQuantity("buck") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'buck', 1)
				TriggerServerEvent('drp-banking:addMoney', math.random(160, 200))
			else
...
AddEventHandler("hunting:level3", function()
	if exports["drp-inventory"]:getQuantity("boar") >= 1 then
		playerAnim()
		local finished = exports["drp-taskbar"]:taskBar(4000,"Selling Boar",true,false,playerVeh)
		if finished == 100 then
			if exports["drp-inventory"]:getQuantity("boar") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'boar', 1)
				TriggerServerEvent('drp-banking:addMoney', math.random(350, 500))
			else
...
--     exports['drp-menuinteract']:AddButton("Sell Coyotepelt", "Wow you suck", "hunting:level1", 'example button 1', "jobmenu")
--     exports['drp-menuinteract']:AddButton("Sell Buck", "Wow you kinda okay", "hunting:level2", 'example button 2', "jobmenu")
--     exports['drp-menuinteract']:AddButton("Sell Boar", "Wow you good", "hunting:level3", 'example button 3', "jobmenu")
...
RegisterNetEvent('shop:casino')
AddEventHandler('shop:casino', function()
	TriggerEvent("server-inventory-open", "299", "Shop")
	Wait(1000)
end)
