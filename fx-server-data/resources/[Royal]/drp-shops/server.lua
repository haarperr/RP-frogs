...
AddEventHandler("itemMoneyCheck", function(itemType,askingPrice,location)
	...
	if (user:getCash() >= askingPrice) then
		...
		if askingPrice > 0 then
			TriggerClientEvent('drp-notification:client:Alert', src, {style = 'info', duration = 3000, message = "Purchased"})
		end
...
AddEventHandler("shop:useVM:server", function(locatoion)
	...
	if (tonumber(pCash) >= 20) then
		...
		TriggerClientEvent('drp-notification:client:Alert', src, {style = 'error', duration = 3000, message = "You need 20$"})
	end
end)
...
AddEventHandler("take100", function(tgtsent)
    local user = exports["drp-base"]:getModule("Player"):GetUser(tgtsent)
    local char = user:getCurrentCharacter()
...
RegisterServerEvent("drp-shops:WeaponShopStatus")
AddEventHandler("drp-shops:WeaponShopStatus", function()
	local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	exports.ghmattimysql:execute("SELECT status FROM user_licenses WHERE cid = @cid AND type = @type ",{['@cid'] = char.id, ['@type'] = 'weapon'}, function(result)
