...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("drp-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
function Login.SelectedChar(data)
	...
	local events = exports["drp-base"]:getModule("Events")
	events:Trigger("drp-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	        ...
	        TriggerEvent("drp-base:firstSpawn")
	    end
    end)
end
...
RegisterNetEvent("drp-spawn:finishedClothing")
AddEventHandler("drp-spawn:finishedClothing", function(endType)
    ...
    if distance <= 10 then
    	...
    	if endType == "Finished" then
            DoScreenFadeOut(2)
    		spawnChar()
            TriggerServerEvent("drp-spawn:licenses")
    	else
    		backToMenu()
    	end
    end	
