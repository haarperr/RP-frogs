...
AddEventHandler('spawn:clientSpawnData', function(spawnData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				-- TriggerEvent("drp-spawn:firstspawnyo")
			end
			return
		end
		...
		--[[if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["drp-housing"]:retriveHousingTable();
		end]]
...
RegisterNetEvent("drp-spawn:firstspawnyo")
AddEventHandler("drp-spawn:firstspawnyo", function()
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then
        TriggerEvent("drp-spawn:finishedClothingstuffbish")
    else
        TriggerEvent("drp-spawn:finishedClothingstuff")
    end
end)

RegisterNetEvent('drp-spawn:finishedClothingstuff')
AddEventHandler('drp-spawn:finishedClothingstuff', function()
	...
	Citizen.CreateThread(function()
        while true do
			...
			if IsCutsceneActive() then 
				...
				Wait(3000)
				FreezeEntityPosition(GetPlayerPed(-1), false)
				if not exports["drp-inventory"]:hasEnoughOfItem("newaccountbox",1,false) then
...
RegisterNetEvent('drp-spawn:finishedClothingstuffbish')
AddEventHandler('drp-spawn:finishedClothingstuffbish', function()
