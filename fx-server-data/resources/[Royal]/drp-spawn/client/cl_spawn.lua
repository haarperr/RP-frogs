...
function Login.Init()
	CreateThread(function()
		...
		local station = GetHashKey("sp_01_station")

      
        
        RequestModel(station)
...
function Login.NUICallback(data)
	...
	if data.action == "newCharacter" then
		...
		events:Trigger("drp-base:createCharacter", chardata, function(created)
            if not created then
...
RegisterNetEvent("drp-base:spawnInitialized")
AddEventHandler("drp-base:spawnInitialized", function()
	Login.nativeStart()
end)
