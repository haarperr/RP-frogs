...
function correct(){
    ...
    $.post('https://drp-ui/drp-ui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
const CloseMenu = () => {
    ...
    saved = "";
    textboxactive = false
    $.post(`https://drp-ui/guccimanecancel`)
...
const PostData = (data) => {
    return $.post(`https://drp-ui/`+textmenucallbackurl, JSON.stringify(data))
}

function closesettings(){
    $(".settings").hide();
    $.post('https://drp-ui/close');
}
                    ...
                    $.each(payload.options, function (index2, item2) {
                        if(index1 == index2 && item2 == true){
                            ...
                            $("#target-"+index1).hover((e)=> {
                                $("#target-"+index1).css("color",e.type === "mouseenter"?"#4F7CAC":"white")
                            })
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://drp-ui/drp-ui:targetSelectOption', JSON.stringify({
            entity : entitydata,
            option :{
                event: eventData,
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        if (contextmenuactive) {
            ...
            $(".buttoncontext").html("");
            $(".containercontext").hide();
            $.post('https://drp-ui/closecontext', JSON.stringify({}));
        }
        if (textboxactive) {
            CancelMenu();
        }
        if (eyeintactive){
            eyeintactive = false
            $.post('https://drp-ui/closeeyeint', JSON.stringify({}));
        }
    }
});
        ...
        if (event.target.id == "backmenu") {
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (children) {
                    ...
                    $(".buttoncontext").html("");
                    $(".containercontext").hide();
                    $.post('https://drp-ui/'+action, JSON.stringify({
                        key:key
                    }));
                ...
                $(".buttoncontext").html("");
                $(".containercontext").hide();
                $.post('https://drp-ui/'+subaction, JSON.stringify({
                    key:subkey
                }));
            }
        }
