        ...
        if s["ongoingHeist"] == false then
            if exports["drp-duty"]:LawAmount() >= copCount then
                ...
                function()
                    ...
                    local bomba = CreateObject(GetHashKey("hei_prop_heist_thermite"), currentPos.x, currentPos.y, currentPos.z+ 0.3,  true,  true, false)
                    ...
                    TriggerServerEvent("drp-ud:particleserver", currentPos.x, currentPos.y, currentPos.z + 0.3)
...
AddEventHandler("drp-ud:doThermite", function(vec4)
    ...
    function()
        ...
        DetachEntity(bomba, 1, 1)
        FreezeEntityPosition(bomba, true)
        TriggerServerEvent("drp-ud:particleserver", vec4.x, vec4.y, vec4.z)
        SetPtfxAssetNextCall("scr_ornate_heist")
