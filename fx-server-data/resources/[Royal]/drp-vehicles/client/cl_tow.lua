...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'towtruck', 1.0)
		    TaskTurnPedToFaceEntity(PlayerPedId(), vehicle, 1.0)
		    Citizen.Wait(1000)
		    TriggerEvent("animation:tow")
		    local finished = exports["drp-taskbar"]:taskBar(15000,"Hooking up vehicle.")
        	if finished == 100 and aDist < 5.0 then
			
				if targetVehicle ~= 0 then
					TriggerEvent('InteractSound_CL:PlayOnOne', 'towtruck2', 1.0)
					local part2 = exports["drp-taskbar"]:taskBar(7000,"Towing Vehicle")
		    ...
		    TriggerEvent("animation:tow")
			TriggerEvent('InteractSound_CL:PlayOnOne', 'towtruck', 1.0)
		    local finished = exports["drp-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then

        		TriggerEvent('InteractSound_CL:PlayOnOne', 'towtruck2', 1.0)
        		local part2 = exports["drp-taskbar"]:taskBar(7000,"Unhooking Vehicle")
...
RegisterNetEvent("drp-vehicles:repo:success")
AddEventHandler("drp-vehicles:repo:success", function()
...
RegisterNetEvent("drp-vehicles:repo:success2")
AddEventHandler("drp-vehicles:repo:success2", function()
	local veh = GetVehiclePedIsIn(PlayerPedId())
	if veh ~= 0 then
		DeleteVehicle(veh)
		DeleteEntity(veh)
		TriggerEvent('DoLongHudText', 'You have successfully impounded the vehicle!', 1)
		TriggerServerEvent('payslip:add', math.random(500,1250))
	end
end)

RegisterNetEvent("search:list:repotow")
AddEventHandler("search:list:repotow", function()
	local job = exports["isPed"]:isPed("myJob")
	if job == 'pdm' or job == 'towunion' or job == 'sanders' then
        local valid = exports["drp-applications"]:KeyboardInput({
            header = "Search Reposession List",
            rows = {
                {
                    id = 0,
                    txt = "Plate Number"
        ...
        if valid then
            TriggerServerEvent("drp-vehicles:checkrepo", valid[1].input)
        end
	else
...
RegisterNetEvent("search:list:reporetrieve")
AddEventHandler("search:list:reporetrieve", function()
	local job = exports["isPed"]:isPed("myJob")
	if job == 'pdm' or job == 'towunion' or job == 'sanders' then
        local valid = exports["drp-applications"]:KeyboardInput({
            header = "Remove Reposession",
            rows = {
                {
                    id = 0,
                    txt = "Plate Number"
        ...
        if valid then
            TriggerServerEvent("drp-vehicles:release:vehicle", valid[1].input)
        end
	else
		TriggerEvent('DoLongHudText', 'You cannot do this!', 2)
	end
...
RegisterNetEvent("search:list:reporepo")
AddEventHandler("search:list:reporepo", function()
	local job = exports["isPed"]:isPed("myJob")
	if job == 'pdm' or job == 'towunion' or job == 'sanders' then
        local valid = exports["drp-applications"]:KeyboardInput({
        ...
        if valid then
            TriggerServerEvent("drp-vehicles:judgerepo", valid[1].input)
        end
	else
		TriggerEvent('DoLongHudText', 'You cannot do this!', 2)
	end
...
RegisterNetEvent("drp-vehicles:towBill")
AddEventHandler("drp-vehicles:towBill", function(amount)
