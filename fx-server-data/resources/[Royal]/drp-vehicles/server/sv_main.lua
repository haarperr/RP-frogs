...
AddEventHandler("mech:add:materials", function(materials, amount, shop)
    ...
    if tonumber(amount) > 0 then
        AddMats(src, materials, tonumber(amount), shop)
        TriggerClientEvent('inventory:removeItem',src, string.lower(materials), tonumber(amount))
        Citizen.Wait(200)
...
AddEventHandler("mech:remove:materials", function(materials, amount, deg, plate, shop)
    local src = source
    if not amount then
        TriggerClientEvent('DoLongHudText', src, 'Error: You need to put a vaild amount', 2)
        return
    end
...
function UpdateDBMats(srcID, part, amount, shop, deg, plate)
    ...
    exports.ghmattimysql:execute("SELECT * FROM mech_materials WHERE Shop = ?",{shop}, function(data)
        if data[1][mat] > tonumber(0) then
            ...
            TriggerClientEvent('DoLongHudText', srcID, "You do not have enough " ..mat.." to finish the repair!", 2)
        end
    end)
end
...
function AddMats(srcID, part, amount, shop)
    ...
    exports.ghmattimysql:execute("SELECT * FROM mech_materials WHERE Shop = ?",{shop}, function(data)
        ...
        TriggerClientEvent('DoLongHudText', srcID, "Shop inventory updated!", 1)
    end)
end
...
AddEventHandler("transfer:attempt:send", function(plate, target)
    local pSrc = source
    local pUser = exports["drp-base"]:getModule("Player"):GetUser(pSrc)
    local pChar = pUser:getCurrentCharacter().id
    local pSteam = GetPlayerIdentifiers(pSrc)[1]

    -- target shit
    local tUser = exports["drp-base"]:getModule("Player"):GetUser(target)
