...
RegisterNetEvent('drp-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('drp-voice:setTalkingOnRadio', setTalkingOnRadio)
...
function addPlayerToRadio(plySource)
	TriggerEvent('drp-voice:getRadioChannel')
...
RegisterNetEvent('drp-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('drp-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('drp-voice:setPlayerRadio', channel)
...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
			TriggerServerEvent('drp-voice:setTalkingOnRadio', true)
			radioPressed = true
			playMicClicks(true)
			...
			Citizen.CreateThread(function()
				TriggerEvent('drp-hud:transmitting', radioPressed)
				TriggerEvent("drp-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		playerTargets(NetworkIsPlayerTalking(PlayerId()) and callData or {})
		TriggerEvent("drp-voice:radioActive", false)
		TriggerEvent('drp-hud:transmitting', radioPressed)
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 1) == 1 then
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
		end
		TriggerServerEvent('drp-voice:setTalkingOnRadio', false)
	end
end, false)
...
RegisterNetEvent('drp-voice:clSetPlayerRadio', syncRadio)

RegisterNetEvent("drp-voice:getRadioChannel")
AddEventHandler("drp-voice:getRadioChannel", function()
	TriggerServerEvent('drp-dispatch:GrabRadioChannel', radioChannel)
end)