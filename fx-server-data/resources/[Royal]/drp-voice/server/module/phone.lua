...
function removePlayerFromCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        TriggerClientEvent('drp-voice:removePlayerFromCall', player, source)
    end
...
function addPlayerToCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        ...
        if player ~= source then
            TriggerClientEvent('drp-voice:addPlayerToCall', player, source)
        end
    end
    callData[callChannel][source] = false
    voiceData[source] = voiceData[source] or defaultTable(source)
    voiceData[source].call = callChannel
    TriggerClientEvent('drp-voice:syncCallData', source, callData[callChannel])
end
...
function setPlayerCall(source, callChannel)
    ...
    if GetInvokingResource() then
        ...
        -- changed
        TriggerClientEvent('drp-voice:clSetPlayerCall', source, callChannel)
    end
    voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('drp-voice:setPlayerCall', function(callChannel)
    ...
    if callTbl then
        ...
        for player, _ in pairs(callTbl) do
            if player ~= source then
                ...
                TriggerClientEvent('drp-voice:setTalkingOnCall', player, source, talking)
            end
        end
    else
...
RegisterNetEvent('drp-voice:setTalkingOnCall', setTalkingOnCall)