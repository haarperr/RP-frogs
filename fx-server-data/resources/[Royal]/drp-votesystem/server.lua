...
AddEventHandler('server:givepass', function(pass_type, wrank, cid)
  ...
  local user = exports["drp-base"]:getModule("Player"):GetUser(src)
  local char = user:getCurrentCharacter()
  exports.ghmattimysql:execute("SELECT rank FROM character_passes WHERE cid = @id", {['id'] = cid}, function(result)
...
AddEventHandler('server:givepayGroup', function(groupname, amount, cid)
  ...
  local user = exports["drp-base"]:getModule("Player"):GetUser(src)
  local char = user:getCurrentCharacter()
  exports.ghmattimysql:execute("SELECT bank FROM group_banking WHERE group_type = @id", {['id'] = groupname}, function(data)
...
AddEventHandler('server:gankGroup', function(groupid, amount)
  ...
  if user:getCash() >= tonumber(amount) then
    user:removeMoney(tonumber(amount))
      exports.ghmattimysql:execute("SELECT bank FROM group_banking WHERE group_type = @id", {['id'] = groupid}, function(result)
