...
Citizen.CreateThread(function()
    while true do
        ...
        -- if player is near the warehouse
            ...
            local exitDistance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), warehouses[i]["x"]-9, warehouses[i]["y"], warehouses[i]["z"]-31, true)
            if exitDistance <= 7.5 then
                Draw3DText(warehouses[i]["x"]-9,warehouses[i]["y"],warehouses[i]["z"]-31, "Press [E] to exit")
