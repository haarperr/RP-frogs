...
function SwitchWeaponMode()
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			...
			TriggerEvent("drp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
...
RegisterNetEvent("drp-weapons:client:weaponEquiped")
AddEventHandler("drp-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("drp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("drp-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("drp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)
