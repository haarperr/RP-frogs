RPC.register("drp-wheelfitment_sv:saveWheelfitment",function(pSource,plate,vehiclewheelMods)
	local src = source
...
RPC.register("drp-wheelfitment_sv:getfuckingdata",function(pSource,plyveh,plate)
	local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(src)
	if not user then return end
	local char = user:getCurrentCharacter()
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE cid = @cid AND license_plate = @plate ', { ['@cid'] = char.id, ['plate'] = plate.param}, function(result)
		if (result and result[1]) then
            TriggerClientEvent("drp-wheelfitment_cl:applySavedWheelFitment", src, result[1].wheelfitment, plyveh.param)
        end
    end)
end)
