        ...
        if exports["drp-inventory"]:hasEnoughOfItem("heistlaptop1", 1) then
            if not firstlowerdoor then
                if #(playercoords - firstdoorlowerlocation) < 5.0 then
                    ...
                    Wait(5000)
                    openanim()
                    exports['hacking2']:OpenHahacking2ckingGame(function(Success)
                        if Success then
                            if deleternd <= 100 and deleternd > 20 then
                                TriggerEvent('inventory:removeItem', 'heistlaptop1', 1)
                            end
                            firstlowerdoor = true
                            TriggerServerEvent('drp-doors:changeLock-status', 255, false)
...
AddEventHandler("sikis:LOWERVAULT_REFRESH", function()
    ...
    if firstlowerdoor then
        TriggerServerEvent('drp-doors:changeLock-status', 255, true)
        TriggerServerEvent("drp-doors:changeLock-status", 256, true)
    end
    ...
    if firstpindoor and secpindoor and thirdpindoor and fourthpindoor then
        TriggerServerEvent('drp-doors:changeLock-status', 254, true)
    end
    Wait(2000)
    vaultdoor = false
    firstlowerdoor = false
        ...
        if secpindoor then
            if thirdpindoor then
                if fourthpindoor then
                    TriggerServerEvent('drp-doors:changeLock-status', 254, false)
                    print('all completed')
                else
...
AddEventHandler('dark-vaultrob:lower:laptoppin', function()
    local deleternd = math.random(1,100)
    if exports["drp-inventory"]:hasEnoughOfItem("vcomputerusb", 2) then
        if not laptoppin  then
            if deleternd <= 100 and deleternd > 20 then
                TriggerEvent('inventory:removeItem', 'vcomputerusb', 1)
            end
            exports['vaultpasswordmg']:OpenHackingGame(9, function(success)
                if success then
                    laptoppin = true
                    TriggerEvent('player:receiveItem', 'lvaccesscodes', 1) 
                    TriggerEvent('inventory:removeItem', 'vcomputerusb', 1)
                else
                    laptoppin = false
                end
...
AddEventHandler('dark-vaultrob:lower:firstpindoor', function()
    if not firstpindoor  then
        exports['vaultpasswordmg']:OpenHackingGame(4, function(success)
            if success then
                firstpindoor = true
