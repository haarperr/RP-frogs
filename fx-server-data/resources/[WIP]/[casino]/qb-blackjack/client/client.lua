...
function hideUiOnStart()
	exports['textUi']:HideTextUi('hide')
	exports['casinoUi']:HideCasinoUi('hide') 
	TriggerEvent("drp-context:closeMenu")
end
...
AddEventHandler("BLACKJACK:RequestBets", function(index, _timeLeft)
	...
	Citizen.CreateThread(function()
		...
		while true do Wait(0)
			exports['casinoUi']:DrawCasinoUi('show', "Diamond Casino Blackjack</p>Time Left: "..s2m(timeLeft).."</p>Bet: "..bet.." chips")   
			...
			if IsControlJustPressed(1, 201) then -- ENTER / A
				...
				if canBet then
					...
					TriggerEvent("DoLongHudText", "You dont have enough Chips for the bet.", 2)
				end
			end
		end --  TriggerEvent("DoLongHudText", 
	end)
end)
	...
	while true do
		...
		if not IsEntityDead(playerPed) then
			for i,v in pairs(tables) do
				...
				local distance = GetDistanceBetweenCoords(cord.x, cord.y, cord.z, GetEntityCoords(PlayerPedId()), true)
				if distance < 3.0 then					
					...
					if GetEntityCoords(tableObj) ~= vector3(0.0, 0.0, 0.0) then
						...
						if GetDistanceBetweenCoords(coords, GetEntityCoords(PlayerPedId()), true) < 2.8 and not IsSeatOccupied(coords, 0.5) and canSit then
							...
							if highStakes then
								text = "<b>Diamond Casino Blackjack(High-Limit)</b></p>Press [E] to sit down</p>(Black Casino Chips)"
							else
								text = "<b>Diamond Casino Blackjack</b></p>Press [E] to sit down</p>(Black Casino Chips)"
							end
