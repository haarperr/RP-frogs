...
function SetExports()
    exports["qb-blackjack"]:SetGetChipsCallback(function(source)
        ...
        local Player = QBCore.Functions.GetPlayer(source)
        local Chips = Player.Functions.GetItemByName("blackchip")
    
        if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
            Chips = Chips
        end
    
        return TriggerClientEvent('QBCore:Notify', src, "You have no Black Casino Chips...")
    end) 
    ...
    exports["qb-blackjack"]:SetTakeChipsCallback(function(source, amount)
        ...
        if Player ~= nil then
            Player.Functions.RemoveItem("blackchip", amount)
            TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['blackchip'], "remove", amount)
        end
    end) 
    ...
    exports["qb-blackjack"]:SetGiveChipsCallback(function(source, amount)
        ...
        if Player ~= nil then
            Player.Functions.AddItem("blackchip", amount)
            TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['blackchip'], "add", amount)
        end
    end)
end
