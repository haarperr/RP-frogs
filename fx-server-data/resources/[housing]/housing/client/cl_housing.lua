...
Citizen.CreateThread(function()
	while true do
        ...
        if insidehouse == true then
			...
			-- 		-- exports["drp-ui"]:showInteraction("Enter", "classic")
			...
			elseif clothing["x"] ~= 0.0 and #(vector3(clothing["x"],clothing["y"],clothing["z"]-0.3) - plyCoords) < 1.5 then
				...
				exports["drp-ui"]:showInteraction("Outfits", "classic")
				if IsControlJustReleased(2, 38) then
			...
			elseif storage["x"] ~= 0.0 and #(vector3(storage["x"],storage["y"],storage["z"]-0.3) - plyCoords) < 1.5 then
				...
				exports["drp-ui"]:showInteraction("[E] Stash", "classic")
				if IsControlJustReleased(2, 38) then
...
AddEventHandler('housing:findsalecid', function(checkcid,price,house_id,house_model,storage,clothing,garages)
	...
	TriggerEvent("DoLongHudText", "The payment is $" .. upfront .. " , if you would like to cancel this dont do /confirm", 1)
...
RegisterKeyMapping('enter', 'Enter', 'keyboard', 'e')
		...
		if myhouseid == 0 then
			...
			if entryType == 1 then
				if myhouseid ~= 0 then
					...
					elseif trailer then
						if (#(vector3((curHouseCoords["x"]-1.443),(curHouseCoords["y"]-1.892),(curHouseCoords["z"]-74.0)) - playerCoords) < 1.2 ) then
...
RegisterNetEvent('drp-motels:RealtorPay')
AddEventHandler('drp-motels:RealtorPay', function(amount)
