...
function polyHelperEnter(apartmentLocNumber,apartmentNumber)
    ...
    listening = true
    exports['drp-textui']:showInteraction(getInteractionMessage(apartmentLocNumber,apartmentNumber))
...
function showApartmentMenu(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = "Apartment # "..k, action = "drp-ui:apartmentsContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
    ...
    local data = {
        {
            ...
            action = "drp-ui:apartmentsContext"
        },
        {
            title = "Apartments",
...
RegisterUICallback("drp-ui:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        --exports["drp-ui"]:showInteraction(getInteractionMessage(globalLocNumber,globalNumber))
    end
...
function showApartmentMenuTwo(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = "Apartment # "..k, action = "drp-ui:apartmentsenterContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
    ...
    if myjob == "police" or myjob == "judge" or myjob == "sheriff" or myjob == "state" then
        ...
        for k,v in pairs(currentApartments) do
            ...
            allApartments[#allApartments + 1] = {
                ...
                description = "Lockdown a Given Apartment.",
                key = {"lockdown",apartmentLocNumber,k},
                action = "drp-ui:apartmentsenterContext"
            }
        end
...
RegisterUICallback("drp-ui:apartmentsenterContext", function(data, cb)
    local keyData = data.key
    local action = keyData[1]
    local loc = keyData[2]
    local room = keyData[3] 
...
RegisterNetEvent('drp-apps:unlocks1')
AddEventHandler('drp-apps:unlocks1', function()
...
RegisterNetEvent('drp-apps:unlocks2')
AddEventHandler('drp-apps:unlocks2', function()
    if globalLocNumber ~= nil then
        showApartmentMenuTwo(globalLocNumber,globalNumber)
    else
        TriggerEvent('DoLongHudText', "No Apartments are in use right now", 1)
