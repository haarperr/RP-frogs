...
AddEventHandler("apartment:serverUnlockAprts",function(roomType)
    ...
    exports.ghmattimysql:execute("select * from character_motel WHERE cid=@cid",{
        ...
        if #result > 0 then
            if result[1].building_type == roomType then
                TriggerClientEvent("drp-apps:unlocks1",src)
            else
                TriggerClientEvent("drp-apps:unlocks2",src)
            end
        end
    end)
end)
...
RegisterServerEvent('drp-apartments:SetNewRouting')
AddEventHandler('drp-apartments:SetNewRouting', function(roomNumber)
    local src = source
    SetPlayerRoutingBucket(src, roomNumber)
end)

RegisterServerEvent('drp-apartments:GetRouting')
AddEventHandler('drp-apartments:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('drp-apartments:SetOldRouting')
AddEventHandler('drp-apartments:SetOldRouting', function()
    local src = source
