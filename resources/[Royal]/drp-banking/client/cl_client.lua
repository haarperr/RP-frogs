...
RegisterNetEvent('drp-banking:UpdatedAccounts')
AddEventHandler('drp-banking:UpdatedAccounts', function(job)
...
function openGui(a)
  ...
  TriggerServerEvent('drp-banking:GetCharacterBanking', cid)
...
RegisterNetEvent('drp-banking:ResetUI')
AddEventHandler('drp-banking:ResetUI', function()
  SetNuiFocus(false, false)
  SendNUIMessage({openBank = false})
  bankOpen = false
  atmOpen = false
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["drp-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["drp-taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterNetEvent("drp-base:addedMoney")
AddEventHandler("drp-base:addedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:addCash", amt)
end)

RegisterNetEvent("drp-base:removedMoney")
AddEventHandler("drp-base:removedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
