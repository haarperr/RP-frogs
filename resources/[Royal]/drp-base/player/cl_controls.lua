...
DRP.Controls.secondaryBinds = {}

DRP.Controls.Distcheck = {}
DRP.Controls.Distcheck.use = {
	-- create fake id
...
DRP.Controls.Distcheck.useSecondary = {
...
DRP.Controls.Distcheck.useThird = {
	-- hospital garage
...
function DRP.DataControls.encodeSetBindTable(self, bindTable)
	...
	DRP.DataControls.setBindTable(controlTable,true)
end

function DRP.DataControls.toUpper(table)
...
function DRP.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		DRP.Controls.Current  = DRP.DataControls.toUpper(DRP.Controls.Default) 
		DRP.DataControls.setSecondaryBindTable(DRP.Controls.Current)
		TriggerServerEvent('drp-base:sv:player_control_set',DRP.Controls.Current)
		DRP.DataControls.checkForMissing()
	else
		if shouldSend then 
			DRP.Controls.Current = DRP.DataControls.toUpper(controlTable)
			DRP.DataControls.setSecondaryBindTable(DRP.Controls.Current)
			TriggerServerEvent('drp-base:sv:player_control_set',DRP.Controls.Current)
			DRP.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",DRP.DataControls.getTableInKeyNumbers())
end

function DRP.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#DRP.Controls.secondaryBinds  do
				if DRP.Controls.secondaryBinds[i] and DRP.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				DRP.Controls.secondaryBinds[i] = {}
				DRP.Controls.secondaryBinds[i][1] = control1
				DRP.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function DRP.DataControls.getTableInKeyNumbers()
...
function DRP.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(DRP.Controls.Current) do
		if k == bindID then
			found = v
...
function DRP.DataControls.distanceCall(distcheckName)
	local distanceTable = DRP.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if DRP.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-DRP.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			DRP.Controls.EventHolder[key] = GetGameTimer();
...
function DRP.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(DRP.Controls.Default) do
		if DRP.Controls.Current[j] == nil then
			isMissing = true
			DRP.Controls.Current[j] = h
		end
	end
...
function DRP.DataControls.validEvent(event,id,eventID)
	local v = DRP.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if DRP.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-DRP.Controls.EventHolder[eventKey])
	end

	if DRP.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			DRP.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if DRP.Controls.Current["tokoptt"] then
			for i=0,#DRP.Controls.events["general"] do
				local v = DRP.Controls.events["general"][i]
				local keyString = DRP.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#DRP.Controls.secondaryBinds do
							if IsControlPressed(1,DRP.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,DRP.Controls.secondaryBinds[i][1]) then
								isConflicted = DRP.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								DRP.DataControls.validEvent(eventBind,idEvent,i)
							else
								DRP.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#DRP.Controls.secondaryBinds do
									if IsControlPressed(1,DRP.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,DRP.Controls.secondaryBinds[i][2]) then
										isConflicted = DRP.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if DRP.Controls.Current["tokoptt"] then
			for i=0,#DRP.Controls.events["general-dist"] do
				local v = DRP.Controls.events["general-dist"][i]
				local key = Keys[DRP.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					DRP.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("drp-base:cl:player_control")
AddEventHandler("drp-base:cl:player_control", function(controlTable)
	DRP.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('drp-base:setcontrols')
AddEventHandler('drp-base:setcontrols', function()
	TriggerServerEvent('drp-base:sv:player_controls')
end)
