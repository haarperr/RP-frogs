...
$(function () {
  function display(bool) {
    if (bool) {
      ...
      $.post("https://drp-boosting/getXp", JSON.stringify({}));
    } else {
  ...
  document.onkeyup = function (data) {
    if (data.which == 27) {
      $.post("https://drp-boosting/close", JSON.stringify({}));
    }
  };
  ...
  $("#queueBtn").click(function () {
    if (isInQueue === false) {
      ...
      $("#queueBtn").hide();
      setTimeout(() => {
        $.post("https://drp-boosting/joinBoostQueue"), JSON.stringify({});
        $("#leaveQueueBtn").show();
        $("#loading").hide();
        isInQueue = true;
      },);
  ...
  $("#leaveQueueBtn").click(function () {
    if (isInQueue === true) {
      ...
      $("#leaveQueueBtn").hide();
      setTimeout(() => {
        $.post("https://drp-boosting/leaveBoostQueue"), JSON.stringify({});
        $("#queueBtn").show();
        $("#loading").hide();
        isInQueue = false;
      },);
  ...
  $("#cancelBoost").click(function () {
    $.post("https://drp-boosting/cancelContract", JSON.stringify({}));
    $("#boostAppMain").show();
    $("#boostInProgress").hide();
    isBoostActive = false;
    // ALSO LOSE XP?
...
// New
  ...
  return
    ...
    if(task === 'start') {
        ...
        if(idsvin.indexOf(id) >= 0) {
            ...
            close()
            $.post('https://drp-boosting/dick', JSON.stringify({
                id: id,
            }));
        }
        ...
        $.post('https://drp-boosting/decline', JSON.stringify({
            id: id,
        }));
    }
  ...
  $("#background-apply").click(function() {
    $.post('https://drp-boosting/updateBoostBackground', JSON.stringify({url: $("#bacgkroundurl").val()}));

    $(".tablet-container").css('background-image' ,"url(" +  $("#bacgkroundurl").val() + ")")
  })

  $("#background-reset").click(function() {
      $.post('https://drp-boosting/updateuupdateBoostBackgroundrl', JSON.stringify({url: dddurl}));
