	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
			...
			if IsControlJustReleased(0,38) then
				...
				local finished = exports["drp-taskbar"]:taskBar(2000,"Making An Exchange")
...
AddEventHandler('drp-polyzone:enter', function(name)
    if name == "rolex_run_start" then
		...
		if not RolexRun then
			exports["drp-textui"]:showInteraction("[E] Start Run ($500)")
		else 
			exports["drp-textui"]:showInteraction("Finish Your Run")
		end
    end
end)
...
function RolexLocation()
	Citizen.CreateThread(function()
        while RolexSpot do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
				if exports['drp-inventory']:hasEnoughOfItem('rolexwatch', 1) or exports['drp-inventory']:hasEnoughOfItem('stolenpsp', 1) or exports['drp-inventory']:hasEnoughOfItem('stolens8', 1) or exports['drp-inventory']:hasEnoughOfItem('stolenBrokenGoods', 1) or exports['drp-inventory']:hasEnoughOfItem('stolen2ctchain', 1) or exports['drp-inventory']:hasEnoughOfItem('stolenraybans', 1) or exports['drp-inventory']:hasEnoughOfItem('stolencasiowatch', 1) then
					if not RolexRun then
...
function DoDropOff(requestMoney)
	...
	if success then
		...
		TriggerEvent("drp-hud:updateStress",true,25)
	else
