...
function checkElement(settings, element, settingName) {
    if (settings[settingName] == true) {
        $(element).css('color', 'rgb(42, 93, 93)');
        $(element).css('border-color', 'rgb(42, 93, 93)');
...
$(function () {
    ...
    $('.door').on('click', function () {
        ...
        $.post('http://drp-carmenu/openDoor', JSON.stringify({
            doorIndex: doorIndex
        })
        );
    });
    ...
    $('.seat').on('click', function () {
        ...
        $.post('http://drp-carmenu/switchSeat', JSON.stringify({
            seatIndex: seatIndex
        })
        );
    });
    ...
    $('.close').on('click', function () {
        $.post('http://drp-carmenu/escape', JSON.stringify({})
        );
    });

    $('.alldoor').on('click', function () {
        if (frontPanel === true) {
            $.post('http://drp-carmenu/togglealldoor', JSON.stringify({}));
        }
    });

    $('.lockdoors').on('click', function () {
        if (frontPanel === true) {
            $.post('http://drp-carmenu/lockdoors', JSON.stringify({}));
        }
    });

    $('.givekeys').on('click', function () {
        $.post('http://drp-carmenu/givekeys', JSON.stringify({}));
    });

    $('.roof').on('click', function () {
        if (frontPanel === true) {
            $.post('http://drp-carmenu/convertroof', JSON.stringify({}));
        }
    });

    $('.window').on('click', function () {
        var windowIndex = $(this).attr('value');
        $.post('http://drp-carmenu/togglewindow', JSON.stringify({
    ...
    $('.engine').on('click', function () {
        if (frontPanel === true) {
            $.post('http://drp-carmenu/toggleengine', JSON.stringify({}));
        }
    });
