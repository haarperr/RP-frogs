...
RegisterNetEvent("drp-ems:smallheal")
AddEventHandler("drp-ems:smallheal", function()
    ...
    if closestPlayer == -1 or closestDistance > 2.0 then
		...
		if health > 0 then
			...
			TriggerServerEvent('drp-ems:heal', GetPlayerServerId(closestPlayer))
...
RegisterNetEvent("drp-ems:bigheal")
AddEventHandler("drp-ems:bigheal", function()
	if IsBusy then return end
    ...
    if closestPlayer == -1 or closestDistance > 2.0 then
		...
		if health > 0 then
			...
			TriggerServerEvent('drp-ems:heal2', GetPlayerServerId(closestPlayer))
...
RegisterNetEvent("drp-ems:emsRevive")
AddEventHandler("drp-ems:emsRevive", function()
	if IsBusy then return end
    ...
    if closestPlayer == -1 or closestDistance > 2.0 then
		...
		if IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_a", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_b", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_c", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_d", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_e", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_f", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_g", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_h", 3)then
			local playerPed = PlayerPedId()
				...
				Citizen.Wait(10000)
				ClearPedTasks(playerPed)
				TriggerServerEvent('drp-death:reviveSV', GetPlayerServerId(closestPlayer))
				TriggerEvent('DoLongHudText', 'You have revived '.. GetPlayerName(closestPlayer)..' and earned $50', 1)
				TriggerServerEvent('payslip:add', 250)
				TriggerServerEvent("server:GroupPayment", "ems", 250)
...
RegisterNetEvent('drp-ems:big')
AddEventHandler('drp-ems:big', function(target)
	local playerPed = PlayerPedId()
	local maxHealth = GetEntityMaxHealth(playerPed)
    local health = GetEntityHealth(playerPed)
    local newHealth = math.min(maxHealth, math.floor(health + maxHealth / 10))
