...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('drp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('drp-hospital:client:SyncBleed')
AddEventHandler('drp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('drp-hospital:client:FieldTreatLimbs')
AddEventHandler('drp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('drp-hospital:client:FieldTreatBleed')
AddEventHandler('drp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('drp-hospital:client:ReduceBleed')
AddEventHandler('drp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('drp-hospital:client:ResetLimbs')
AddEventHandler('drp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('drp-hospital:client:RemoveBleed')
AddEventHandler('drp-hospital:client:RemoveBleed', function()
    isBleeding = 0
