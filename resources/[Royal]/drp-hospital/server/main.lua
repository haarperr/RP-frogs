...
RegisterServerEvent('drp-hospital:server:RequestBed')
AddEventHandler('drp-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('drp-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('drp-hospital:server:RPRequestBed')
AddEventHandler('drp-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('drp-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('drp-hospital:server:EnteredBed')
AddEventHandler('drp-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('drp-hospital:client:FinishServices', src)
end)

RegisterServerEvent('drp-hospital:server:LeaveBed')
AddEventHandler('drp-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)