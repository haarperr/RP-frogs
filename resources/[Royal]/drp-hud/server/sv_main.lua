...
RegisterServerEvent('drp-hud:UpdateStress_SV')
AddEventHandler('drp-hud:UpdateStress_SV',function(amount)
	local src = source
	local user = exports["drp-base"]:getModule("Player"):GetUser(src)
	local characterId = user:getCurrentCharacter().id
	exports.ghmattimysql:execute("UPDATE characters SET `stress_level` = ? WHERE id = ?",{amount, characterId})
end)
