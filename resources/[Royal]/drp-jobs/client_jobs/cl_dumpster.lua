...
RegisterNetEvent('drp-dumpster')
AddEventHandler('drp-dumpster', function() 
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, dumpPos.x, dumpPos.y, dumpPos.z, true) then
            ...
            for i = 1, #searched do
                ...
                elseif i == #searched and not dumpsterFound then
                    ...
                    local finished = exports ['drp-taskbar']:taskBar(10000, 'search')
                    if (finished ==100) then
                        TriggerServerEvent('royal:giveDumpsterReward')
                        TriggerServerEvent('royal:startDumpsterTimer', dumpster)
...
RegisterNetEvent('drp-dumpster2')
AddEventHandler('drp-dumpster2', function() 
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dumpsterFound = false
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, dumpPos.x, dumpPos.y, dumpPos.z, true) then
            ...
            for i = 1, #searched do
                ...
                elseif i == #searched and not dumpsterFound then
                    ...
                    if (finished ==100) then
                        TriggerServerEvent('royal:giveDumpsterReward')
                        TriggerServerEvent('royal:startDumpsterTimer', dumpster2)
                        FreezeEntityPosition(PlayerPedId(),false)
                        ClearPedTasks(PlayerPedId())
                    end
...
RegisterNetEvent('drp-dumpster3')
AddEventHandler('drp-dumpster3', function() 
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dumpsterFound = false
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, dumpPos.x, dumpPos.y, dumpPos.z, true) then
            ...
            for i = 1, #searched do
                ...
                elseif i == #searched and not dumpsterFound then
                    ...
                    if (finished ==100) then
                        TriggerServerEvent('royal:giveDumpsterReward')
                        TriggerServerEvent('royal:startDumpsterTimer', dumpster3)
                        FreezeEntityPosition(PlayerPedId(),false)
                        ClearPedTasks(PlayerPedId())
                    end
...
RegisterNetEvent('royal:removeDumpster')
AddEventHandler('royal:removeDumpster', function(object)
    for i = 1, #searched do
