...
Citizen.CreateThread(function()
  while true do
    ...
    if dst < 40 and jobSpawned == false then
      TriggerEvent('drp-hunting:spawnJobPed',dumbass, 225.87)
      jobSpawned = true
      refreshPed = true
    end
    if dst >= 41  then
...
RegisterNetEvent('drp-hunting:spawnJobPed')
AddEventHandler('drp-hunting:spawnJobPed',function(coords, heading)
...
RegisterNetEvent('drp-hunting:spawnAnimal')
AddEventHandler('drp-hunting:spawnAnimal', function()
  ...
  if outPosition.x > 1 or outPosition.x < -1  then
    ...
    print("^2[Royal Hunting] Debug: Too Far to Spawn^0")  
  end
...
RegisterNetEvent('drp-hunting:skinAnimal')
AddEventHandler('drp-hunting:skinAnimal', function()
  if exports["drp-inventory"]:getQuantity("huntingknife") > 0  then
    if DoesEntityExist(baitAnimal) then
        LoadAnimDict('amb@medic@standing@kneel@base')
        LoadAnimDict('anim@gangops@facility@servers@bodysearch@')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        TaskPlayAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@" ,"player_search" ,8.0, -8.0, -1, 48, 0, false, false, false )
        local finished = exports["drp-taskbar"]:taskBar(5000,"Skinning")
        if finished == 100 then
          ClearPedTasksImmediately(PlayerPedId())
          DeleteEntity(baitAnimal)
          TriggerServerEvent('drp-hunting:skinReward')
        end
    else
      print("^1[Hunting] Cannot Complete^0") 
    end
  else
...
RegisterNetEvent('drp-hunting:usedBait')
AddEventHandler('drp-hunting:usedBait', function()
  for k,v in pairs (HuntingZones) do
    if IsEntityInZone(GetPlayerPed(-1),v) then
      if cooldown <= 0 then
        LoadAnimDict('amb@medic@standing@kneel@base')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        local finished = exports["drp-taskbar"]:taskBar(5000,"Placing Bait",false,false,playerVeh)
        Citizen.Wait(100)
        cooldown = useBaitCooldown * 10000
        ClearPedTasksImmediately(PlayerPedId())
        used = true
        usedBait()
        TriggerEvent('drp-hunting:spawnAnimal')
        TriggerServerEvent('drp-hunting:removeBait')
        baitCooldown()
      end
    end
  end
...
function usedBait()
  Citizen.CreateThread(function()
    while used do
      print("^1[RPFrogs Hunting] Waiting For Animal To Spawn^0")  
      Wait(30000)
      print("^2[RPFrogs Hunting] Animal Spawned^0")  
      TriggerEvent('drp-hunting:spawnAnimal')
      used = false
    end
  end)
...
RegisterNetEvent('drp-hunting:equip')
AddEventHandler('drp-hunting:equip', function()
...
AddEventHandler("drp-polyzone:enter", function(name)
  ...
  exports['drp-textui']:showInteraction("[E] Sell Animal Carcass")
  listenForKeypress()
end)
...
Citizen.CreateThread(function()
  exports["drp-polyzone"]:AddCircleZone("huntingsales", vector3(12.28, -1299.32, 29.29), 1.3, {
      useZ = true,
      debugPoly= false,
  })
end)