...
RegisterNetEvent('drp-scenes:send', function(sent)
    scenes = sent
end)
...
RegisterCommand('-scenecreate', function()
    ...
    local scene = exports["drp-applications"]:KeyboardInput({
        header = "Add Scene",
        rows = {
            {
                id = 0,
    ...
    TriggerServerEvent('drp-scenes:add', coords, message, color, distance)
end)
...
RegisterCommand('+scenedelete', function()
    ...
    if scene ~= nil then
        TriggerServerEvent('drp-scenes:delete', scene)
    end
end)
...
TriggerServerEvent('drp-scenes:fetch')
