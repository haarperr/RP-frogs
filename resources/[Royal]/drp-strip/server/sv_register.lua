...
AddEventHandler("vanilla:retreive:receipt", function(regID)
  local src = source
  local user = exports["drp-fw"]:getModule("Player"):GetUser(src)
      ...
      for i = 1, #vanilla[regID] do
          if vanilla[regID][i].regID == regID then
              ...
              if (tonumber(user:getCash()) >= tonumber(amount)) then
                  ...
                  local owner = exports["drp-fw"]:getModule("Player"):GetUser(vanilla[regID][i].owner)
                  ...
                  TriggerClientEvent("player:receiveItem", vanilla[regID][i].owner, "burgerReceipt", 1, true, information)
